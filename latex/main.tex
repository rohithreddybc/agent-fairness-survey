...
\subsubsection{From Answering to Acting}
...
\subsubsection{Thesis: Fairness Is a Per-Component, Operator-Mediated Property}
...
\textbf{Contribution 4: A protocol and pilot for an executable cross-framework bias audit.} We specify, and run a small pilot of, a protocol (\S{}6): multiple agent configurations on a consequential-decision task with counterfactual demographic swaps, testing whether component-level action disparity arises that QA-level scoring of the same underlying model would not register. \emph{The pilot illustrates the predicted masking-and-amplification pattern but is directional only; a full confirmatory study is future work.} The protocol, scripts, and raw decisions will be released with the camera-ready.
...
\subsubsection{Positioning and Differentiation}
...
\begin{tabular}{|p{0.090\textwidth}|p{0.090\textwidth}|p{0.090\textwidth}|p{0.090\textwidth}|p{0.090\textwidth}|p{0.090\textwidth}|p{0.090\textwidth}|p{0.090\textwidth}|p{0.090\textwidth}|p{0.090\textwidth}|}
\toprule
\textbf{Survey} & \textbf{Year} & \textbf{Venue} & \textbf{Organizing axis} & \textbf{Agent coverage} & \textbf{Fairness-dedicated} & \textbf{Component-level} & \textbf{Corpus size} & \textbf{Original results} & \textbf{Audit/executable metric?} \\
\midrule
Mehrabi et al. \cite{mehrabi2021survey} & 2021 & ACM CSUR & Bias source $\times$ ML stage & None & Yes & No & ~150 & No & No \\
Gallegos et al. \cite{gallegos2024bias} & 2024 & \emph{Computational Linguistics} & Bias type $\times$ LLM task & Peripheral & Yes & No & ~250 & Partial & No \\
...
\textbf{This survey} & \textbf{2026} & \textbf{ACM CSUR (target)} & \textbf{BCF locus $\times$ operator} & \textbf{Full (all 6 components)} & \textbf{Yes} & \textbf{Yes} & \textbf{202} & \textbf{Protocol specified} & \textbf{Action-level (tool, esc., plan)} \\
\bottomrule
\end{tabular}%
}
\end{table*}
...
\subsubsection{Fairness Definitions}
...
\subsubsection{LLM Bias: A Brief Inventory and the Agentic Gap}
...
\subsubsection{What Is an LLM-Based Agent? Component Anatomy}
...
\subsubsection{Scope and Coverage Decisions}
...
\subsubsection{Systematic Search and Corpus Construction}
...
\begin{longtable}{|p{0.135\textwidth}|p{0.165\textwidth}|p{0.175\textwidth}|p{0.115\textwidth}|p{0.052\textwidth}|p{0.045\textwidth}|p{0.050\textwidth}|p{0.085\textwidth}|}
\caption*{Table 2.1: Search Protocol by Branch. \emph{Takeaway: the branch structure maps directly onto the six BCF entry loci plus evaluation and mitigation; no branch has fewer than 10 identified records before deduplication.}}
\label{tab:2}\\
\toprule
\textbf{Branch} & \textbf{Primary Search String} & \textbf{Supplementary Terms} & \textbf{Databases} & \textbf{Records Identified} & \textbf{After Dedup} & \textbf{After Screen} & \textbf{Included} \\
\midrule
\endfirsthead
\multicolumn{8}{l}{\emph{Table 2.1 (continued)}}\\
\toprule
\textbf{Branch} & \textbf{Primary Search String} & \textbf{Supplementary Terms} & \textbf{Databases} & \textbf{Records Identified} & \textbf{After Dedup} & \textbf{After Screen} & \textbf{Included} \\
\midrule
\endhead
...
\begin{table}[htbp]
\centering\small
\caption*{Table 2.2: Inclusion and Exclusion Criteria. \emph{Takeaway: the criteria operationalize a single principle: fairness as a property of agent actions, not merely of model outputs.}}
\label{tab:3}
\resizebox{\columnwidth}{!}{%
...
\subsubsection{Temporal and Venue Distribution}
...
\textbf{Conduction operator $\phi$$\dagger$}: the scalar multiplier describing how a pipeline edge transforms component-level disparity. $\phi < 1$ = ATTENUATE; $\phi \approx 1$ = PRESERVE; $\phi > 1$ = AMPLIFY; $\phi$ acting on zero input = GENERATE.
...
\textbf{Trajectory disparity $\Delta_\tau$$\dagger$}: the total counterfactual disparity observed over a complete agent trajectory, approximated by the conduction equation $\Delta_\tau \approx \sum_c \Delta_c \cdot \prod_{e > c} \phi_e$ with feedback recurrence $\Delta^{(t+1)} = \phi_{\text{fb}} \cdot \Delta^{(t)} + \Delta_{\text{entry}}$.
...
\subsubsection{Threats to Validity of This Survey}
...
\subsection{The Bias Conduction Framework: Where Bias Enters and How It Travels}
...
\subsubsection{The Framework: Definitions, Operators, Conduction, and Propositions}
...
\textbf{Table 1: Component $\times$ harm mechanism $\times$ representative evidence $\times$ maturity.} Maturity is graded $\bullet$$\bullet$$\bullet$ (well-studied), $\bullet$$\bullet$$\circ$, or $\bullet$$\circ$$\circ$ (emerging).

\begin{table*}[htbp]
\centering\small
\caption*{Table 1: Component $\times$ harm mechanism $\times$ representative evidence $\times$ maturity. Maturity is graded $\bullet$$\bullet$$\bullet$ (well-studied), $\bullet$$\bullet$$\circ$, or $\bullet$$\circ$$\circ$ (emerging).}
\label{tab:6}
\resizebox{\columnwidth}{!}{%
...
\subsubsection{Locus C1: Tool / API Selection Bias}
...
\subsubsection{Locus C2: Memory \& Retrieval: Bias Injection and Accumulation}
...
\subsubsection{Locus C3: Multi-Agent Delegation \& Role Assignment}
...
\textbf{Conduction signature.} \emph{Entry:} the $\pi_{\mathrm{ctrl}}$$\rightarrow$\{A\_i\} assignment interface. \emph{Observable:} the role/subtask assignment matrix across counterfactual pairs, plus inter-agent message content. \emph{Dominant outbound operators:} GENERATE (persona injection) followed by AMPLIFY (interaction). C3 is the only locus where both agentic operators are corpus-evidenced together. \emph{Metric:} assignment disparity over the delegation matrix (\S{}4.3).

\subsubsection{Locus C4: Planning / Decomposition Disparities}
...
\subsubsection{Locus C5: User Modeling \& Personalization Harms}
...
\subsubsection{The Temporal Axis: Long-Horizon Fairness Drift Over Trajectories}
...
\subsubsection{Compounding Across the Pipeline: Proposition P3 and Its Limits}
...
\subsubsection{Worked Example: A Hiring-Screener Agent Through the BCF, and a Four-Step Auditor Recipe}
...
\textbf{Locus} & \textbf{signed $\Delta_c$} & \textbf{edge operator} & \textbf{$\varphi_e$} & \textbf{endpoint contribution} \\
\midrule
Delegation/assignment & +0.15 & AMPLIFY & 1.6 & 0.24 \\
...
\subsubsection{Trajectory-Level Counterfactual Metrics: CFR and MASD as Instances of $\Delta_c$}
...
\subsubsection{Action-Level Disparity Metrics: Formal Definitions}
...
\subsubsection{Datasets and Benchmarks: An Inventory of the Gap}
...
\begin{longtable}{|p{0.205\textwidth}|p{0.110\textwidth}|p{0.150\textwidth}|p{0.180\textwidth}|p{0.075\textwidth}|p{0.085\textwidth}|}
\caption*{Table 2. Fairness evaluation resources and agent capability harnesses.}
\label{tab:9}\\
\toprule
\textbf{Benchmark / resource} & \textbf{Fairness level} & \textbf{Domain} & \textbf{Primary metric} & \textbf{Agentic?} & \textbf{Code/Data released?} \\
\midrule
\endfirsthead
\multicolumn{6}{l}{\emph{Table 2 (continued)}}\\
\toprule
\textbf{Benchmark / resource} & \textbf{Fairness level} & \textbf{Domain} & \textbf{Primary metric} & \textbf{Agentic?} & \textbf{Code/Data released?} \\
\midrule
\endhead
...
\subsubsection{Prompting and In-Context Debiasing}
...
\subsubsection{Guardrails and Filters}
...
\subsubsection{Multi-Agent Debiasing}
...
\subsubsection{Process-Level, Architectural, and RAG-Level Interventions}
...
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{longtable}{|p{0.150\textwidth}|p{0.155\textwidth}|p{0.130\textwidth}|p{0.150\textwidth}|p{0.140\textwidth}|p{0.075\textwidth}|}
\caption*{Table 3: Mitigation Map: Entry Locus $\times$ Conduction Operator $\times$ Pipeline Stage $\times$ Evidence}
\label{tab:10}\\
\toprule
\textbf{Entry Locus} & \textbf{Conduction Operator Targeted} & \textbf{Pipeline Stage} & \textbf{Intervention Class} & \textbf{Representative Evidence} & \textbf{Evidence Density} \\
\midrule
\endfirsthead
\multicolumn{6}{l}{\emph{Table 3 (continued)}}\\
\toprule
\textbf{Entry Locus} & \textbf{Conduction Operator Targeted} & \textbf{Pipeline Stage} & \textbf{Intervention Class} & \textbf{Representative Evidence} & \textbf{Evidence Density} \\
\midrule
\endhead
...
\caption*{Table 4: Challenges $\rightarrow$ Solutions $\rightarrow$ Gaps: Closing the \S{}3 $\rightarrow$ \S{}5 $\rightarrow$ \S{}8 Loop}
\label{tab:11}\\
\toprule
...
\midrule
\endfirsthead
\multicolumn{5}{l}{\emph{Table 4 (continued)}}\\
\toprule
\textbf{Component (\S{}3.x)} & \textbf{Primary Harm (BCF operator)} & \textbf{Mitigation Evidence} & \textbf{Coverage} & \textbf{Critical Open Gap ($\rightarrow$ \S{}8)} \\
\midrule
\endhead
...
\subsection{Empirical Study: A Cross-Framework Bias Audit: Protocol and Design}
...
\textbf{The protocol is fully specified below; Section 6.5 reports a small pilot run. The pilot is directional only, with a small sample and deliberately narrow scope, and no claim in this manuscript outside \S{}6.5 rests on its results.}
...
\item \emph{Escalation/deferral disparity} ($\Delta_c$ at routing): differential rates of human escalation or information-request, a channel where P2 Masking operates at the routing level.
\end{itemize}
...
\textbf{H1: P2 Masking is real.} In at least one configuration, answer-level CFR/MASD is small or statistically indistinguishable from C0 while action-level $\Delta_c$ is materially larger \cite{li2025actions}.

\textbf{H2: Amplification ordering (a proxy for P3, since the pilot does not estimate the component sum).} Total trajectory disparity is non-decreasing along C0 $\preceq$ C1 $\preceq$ C2 $\preceq$ C3, with C3 exhibiting the largest magnitude, consistent with multi-agent amplification \cite{madigan2025emergent,nguyen2025social,cao2026from}. A registered counter-hypothesis: C3 debate can reduce disparity via anonymization \cite{choi2025identity} or ensemble debiasing \cite{owens2024multi}; the sign of the C3 effect is an open empirical question.

Statistical treatment: paired contrasts, 95\% confidence intervals, Bonferroni correction across the configuration $\times$ cue-channel grid; null results reported as such.

\subsubsection{The Role of This Audit}
...
\subsubsection{Results: A Pilot Run}

We report a \emph{pilot} instantiation of the protocol, deliberately small and explicitly bounded in what it can support. The model under test is Claude Haiku 4.5 at temperature 0; the task is resume screening over the 10 synthetic profiles of \S{}6.1; the counterfactual is a single name pair, a White-male-coded name and a Black-female-coded name in the lineage of \cite{veldanda2023emily}, applied to identical resume content; and three configurations are run: C0 (single call), C2 (reason-then-decide), and C3 (a two-role recruiter-reviewer deliberation within one context). This pilot instantiates the answer-level statistics (CFR, MASD) and the decision-action disparity; the tool-invocation and escalation metrics of \S{}6.2, which require a tool-using deployment, are left to the full study.
...
\begin{tabular}{|p{0.180\textwidth}|p{0.180\textwidth}|p{0.180\textwidth}|p{0.180\textwidth}|p{0.180\textwidth}|}
\toprule
\textbf{Configuration} & \textbf{n pairs} & \textbf{CFR} & \textbf{MASD (0-100 score)} & \textbf{Advance-rate disparity} \\
\midrule
...
\subsubsection{The Counterfactual Column is Essentially Empty}
...
\emph{Takeaway (Figure~\ref{fig:coverage}): Across all six agent components, counterfactual fairness (the dimension BCF-based auditing requires for $\Delta_c$ estimation) appears in exactly one of the 68 component-branch entries; GENERATE and AMPLIFY conduction signatures remain empirically uncharted.}

\subsubsection{Group Fairness Clusters Only Where Decisions are Explicit}
...
\subsubsection{Tool, Memory, and Personalization Harms are Documented but Rarely Formalized}
...
\subsubsection{The Locus $\times$ BCF-Operator Grid}
...
\caption{Locus $\times$ BCF conduction-operator grid, cells shaded by paper count (single-hue, colorblind-safe blue; row sums total 68). The GENERATE and AMPLIFY columns are sparse: planning/decomposition has no GENERATE entry, and tool selection registers a single GENERATE paper, marking the highest-priority measurement gaps.}
\label{fig:operator}
\end{figure*}

\emph{Takeaway (Figure~\ref{fig:operator}): GENERATE and AMPLIFY cells (the operators that produce distinctly agentic harm, where pipeline edges produce disparity that was absent at the input or that grows across the trajectory) are the least instrumented; the AMPLIFY/GENERATE column for planning/decomposition and the GENERATE column for tool selection are nearly uncharacterized, pointing to the highest-priority measurement gaps for BCF-guided empirical work.}
...
\subsubsection{Synthesis and Differentiation}
...
\subsection{Open Problems and Research Agenda}
...
\subsubsection{Unpopulated P5 Cells: Benchmarks for Acting Agents}
...
\subsubsection{Unmeasured $\varphi_{\mathrm{fb}}$ Recurrence: Long-Horizon and Compounding Fairness}
...
\subsubsection{Unbounded GENERATE: Multi-Agent Fairness Dynamics}
...
\subsubsection{Intersectional and Sociotechnical Harms in Deployment}
...
\subsubsection{Empty Operator Rows of the Conduction Grid: Governance and Standardized Auditing}
...
\subsubsection{Mitigation at the Agent Level, Not the Token Level}
...
\item The pilot instantiates only the answer-level metrics (CFR, MASD) and the decision-action disparity. The remaining action-level metrics (tool-invocation disparity, escalation disparity, plan-allocation disparity) are specified in \S{}6.2 but require a tool-using deployment and have not been computed; their distributional properties, sensitivity, and test-retest reliability are unknown until the full study.
\end{itemize}

These limitations are by design: the survey's contribution is the framework, the protocol, and a first directional measurement, not a confirmatory empirical study. The harness and data are released as a replication target; the full study is the immediate next step (\S{}8.1).
