...
\item \emph{Vatsal et al. 2026} \cite{vatsal2026agentic}: a clinical-agent evaluation taxonomy whose seven dimensions include a Safety \& Ethics dimension under which fairness sits; conceptual and single-domain (healthcare), with no component-level counterfactual and no corpus.
\item \emph{Mayilvaghanan et al. 2026} \cite{mayilvaghanan2026counterfactual}: applies CFR/MASD in a single domain (contact-center agent quality assurance); our survey generalizes those metrics across all six BCF components.
\item \emph{Ranjan et al. 2025} \cite{ranjan2025fairness}: a 12-page position paper on fairness in agentic/multi-agent AI broadly, not specific to LLM-agent components; no component taxonomy, no empirical corpus.
...
Among all surveyed works, this survey is the first to simultaneously: (a) dedicate the entire organizing axis to fairness, (b) structure that axis by agent component (not by metric family, domain, or trust dimension), (c) introduce a named formal framework (BCF) with testable propositions, and (d) deliver action-level disparity metrics and an audit protocol. We state this claim once here; subsequent sections reference Table~\ref{tab:survey-comparison} rather than repeating it.
...
\resizebox{\textwidth}{!}{%
\begin{tabular}{|p{0.110\textwidth}|p{0.060\textwidth}|p{0.115\textwidth}|p{0.105\textwidth}|p{0.085\textwidth}|p{0.075\textwidth}|p{0.070\textwidth}|p{0.070\textwidth}|p{0.085\textwidth}|p{0.080\textwidth}|}
\toprule
\textbf{Survey} & \textbf{Year} & \textbf{Venue} & \textbf{Organizing axis} & \textbf{Agent coverage} & \textbf{\shortstack[l]{Fairness-\\dedicated}} & \textbf{\shortstack[l]{Component-\\level}} & \textbf{\shortstack[l]{Corpus\\size}} & \textbf{\shortstack[l]{Original\\results}} & \textbf{\shortstack[l]{Audit/executable\\metric?}} \\
\midrule
Mehrabi et al. \cite{mehrabi2021survey} & 2021 & ACM CSUR & Bias source $\times$ ML stage & None & Yes & No & ~150 & No & No \\
Gallegos et al. \cite{gallegos2024bias} & 2024 & \emph{Computational Lin\-guis\-tics} & Bias type $\times$ LLM task & Peripheral & Yes & No & ~250 & Partial & No \\
Chu et al. \cite{chu2024fairness} & 2024 & ACM SIGKDD Expl. & Fairness definition $\times$ LLM task & None & Yes & No & ~100 & No & No \\
...
\textbf{Researcher (contributing to the fairness-of-agents literature).} Read the entire survey. The coverage matrix (Figure~\ref{fig:coverage}, \S{}7) visualizes which BCF cells are most underinstrumented, and these are the highest-priority open problems. The BCF propositions in \S{}3.0 are the natural targets for theoretical work; the trajectory-level metric definitions in \S{}4.3 are the natural targets for empirical instantiation; and \S{}8 itemizes concrete open problems with first-step specifications.
...
Orthogonal to the criterion families above is a distinction in the \emph{kind} of harm a disparity inflicts. Following \cite{barocas2023fairness} and the critical framing of \cite{blodgett2020language}, an \emph{allocative} harm withholds or unequally distributes a resource or opportunity (a loan, an interview, an escalation, a tool that unlocks better service), whereas a \emph{representational} harm degrades how a group is depicted, characterized, or modeled (stereotyped personas, demeaning portrayals, distorted user inferences) independently of any immediate allocation. The distinction matters for an acting agent because the two harm types call for different measurements: allocative harm is read off action and decision distributions, representational harm off the content the agent produces or the attributes it imputes. Locating the survey's six components (\S{}2.3) within this distinction, most are predominantly \emph{allocative}: tool/API selection (C1), memory/retrieval (C2), planning/decomposition (C4), and the long-horizon trajectory all gate access to resources and opportunities. Two components \emph{straddle} both: persona and role assignment (C3) can inject representational harm (a demeaning role) that then becomes allocative once the assigned agent acts on it, and user modeling and personalization (C5) imputes attributes (a representational act) that subsequently steer allocative decisions. This dual character is why C3 and C5 recur below as the loci where the GENERATE operator first injects disparity.

\paragraph{Intersectional Fairness}

The single-attribute criteria above also flatten a structural feature of real discrimination: harms accruing at the \emph{intersection} of attributes can exceed what either attribute predicts alone. The legal-theoretic origin of this observation is \cite{crenshaw1989demarginalizing}, who showed that antidiscrimination doctrine analyzing race and sex as separable axes systematically erases the distinct harms borne at their intersection. The corresponding formal criterion is \emph{differential (intersectional) fairness} \cite{foulds2020intersectional}, which requires bounded disparity not just across marginal groups but across their joint subgroups, with a probabilistic guarantee that holds under subgroup composition. This theoretical pairing (Crenshaw's account of why intersections matter and Foulds et al.'s definition of how to measure them) anchors the intersectional extension $\Delta_c^{A \times B}$ that \S{}8.4 develops as a multi-attribute generalization of the BCF's per-component disparity $\Delta_c$.
...
\begin{itemize}
\item \textbf{Bias origins and pitfalls.} \cite{blodgett2021stereotyping} audit fairness benchmark datasets and catalog recurring methodological pitfalls, establishing that measurement artefacts can mislead benchmark-based conclusions. This concern is taken up again in Section 2.8 (Threats to Validity).
\end{itemize}
...
\textbf{Component 6: Long-horizon trajectory execution.} Over extended multi-turn interactions or autonomous task executions, small per-step biases can compound into large cumulative disparities \cite{ma2026implicit,wyllie2024fairness}. This longitudinal dimension is not captured by any existing single-turn benchmark and constitutes the most undertheorized component in current fairness research. We present it here as a sixth conceptual dimension, but \S{}3.6 argues it is more precisely a temporal axis on the feedback edge rather than an independent architectural component, a distinction the BCF makes precise once the framework is in place.

These six components form the organizing axis of the taxonomy in Section 3. The BCF framework formalizes how bias propagates across them. A fast-growing and especially consequential agent class cuts across all six: \emph{computer-use / GUI agents} that perceive a screen and act through clicks, keystrokes, and form fills in real operating-system and web environments, the setting OSWorld \cite{xie2024osworld} establishes as a reproducible execution substrate. Because such agents take consequential trajectory-level actions (filing forms, submitting applications, navigating benefits portals), they are the natural home of \emph{action disparity}: a demographic signal can steer which control an agent clicks or which path it takes through an interface, a disparity that is invisible at the token level and only observable over the action trajectory. Two classes of agent evaluation benchmarks assess the broader capabilities of agents without a fairness lens, and both must be acknowledged here: WebArena \cite{zhou2024webarena}, AgentBench \cite{liu2024agentbench}, SWE-bench \cite{jimenez2024swe}, Mind2Web \cite{deng2023mind2web}, OSWorld \cite{xie2024osworld}, and the tool-agent-user benchmark $\tau$-bench \cite{yao2024taubench} evaluate task completion or policy compliance in realistic environments and could, in principle, be demographically instrumented; their current designs do not include demographic counterfactual conditions, which is precisely the gap Section 4.3 formalizes as action-level disparity metrics.
...
The corpus was assembled through a systematic multi-branch search following PRISMA-style conventions \cite{moher2009preferred}. Search queries were issued across the following databases: arXiv (cs.AI, cs.CL, cs.LG, cs.IR), ACL Anthology, ACM Digital Library, NeurIPS\allowbreak/\allowbreak ICML\allowbreak/\allowbreak ICLR proceedings archives, FAccT proceedings, AAAI proceedings, and IEEE Xplore, supplemented by forward and backward citation chasing from anchor papers. Searches were conducted between January 2026 and June 2026; the literature cutoff date is \textbf{June 10, 2026}. Papers published after that date are not included.

Ten of the eleven branches received a dedicated search string constructed from the intersection of (a) an agent/agentic qualifier and (b) a fairness/bias qualifier, with branch-specific terms (the six component branches, plus evaluation, mitigation, high-stakes domains, and a prior-art neighbor-survey search). The remaining branch (foundations and adjacent evidence) carries no primary search string because it was populated by backward citation chasing rather than independent keyword search; to make this branch reproducible, the chase was seeded from a fixed set of anchor papers, namely the classical fairness definitions and impossibility results \cite{dwork2012fairness,hardt2016equality,kusner2017counterfactual}, the ML-fairness survey baseline \cite{mehrabi2021survey,barocas2023fairness}, and the two parent LLM-fairness surveys \cite{gallegos2024bias,chu2024fairness}, with one backward hop taken from each anchor's reference list and the results screened under the same inclusion criteria as the keyword branches.
...
\textbf{Stage 4: Inclusion (n = 202).} Full-text review confirmed inclusion criteria and assigned branch tags. Records excluded at this stage failed criterion (iii), typically single-turn generation studies with no agentic framing, or were superseded by a more recent version of the same work. The final corpus comprises 202 unique papers spanning 2012 to 2026.
...
The agent fairness literature is a recent and rapidly growing subfield. Of the 202 corpus papers, \textbf{18 (9\%)} predate 2020 (primarily foundational ML fairness and NLP bias work), \textbf{22 (11\%)} fall in 2020 to 2022, \textbf{31 (15\%)} in 2023, \textbf{78 (39\%)} in 2024, and \textbf{53 (26\%)} in 2025 to 2026 (through June cutoff). The combined 2024 to 2026 cohort constitutes \textbf{65\%} of the corpus (131 papers), reflecting the field's rapid emergence following the widespread deployment of instruction-tuned and tool-augmented LLMs.
...
\begin{enumerate}
\item \textbf{ReAct (late 2022, published ICLR 2023 \cite{yao2023react}).} The formalization of reasoning-and-acting brought agentic behavior into mainstream NLP evaluation. A measurable uptick in tool-selection and planning fairness papers appears in the 2023 to 2024 cohort, as researchers applied bias probing to the new approach.
\end{enumerate}
...
\begin{enumerate}
\item \textbf{GPT-4 and the clinical deployment wave (2023 to 2024).} The application of frontier models to clinical decision support \cite{omar2025sociodemographic,poulain2024bias,zack2024coding} triggered a wave of domain-specific fairness audits that now constitute the high-stakes-domains branch.
\end{enumerate}
...
Several observations follow. The arXiv share is high (31\%), reflecting the recency of the field: many 2025 to 2026 papers are under review or in proceedings preparation. The ACM FAccT representation (9 papers) is lower than the field's importance would suggest, indicating that agent fairness has not yet been fully claimed by the accountability community; it currently sits across NLP venues, systems venues, and domain journals. Ten papers appear in CSUR or ACM Computing Surveys directly, indicating that the survey genre has begun to engage this territory, and the differentiation analysis in \S{}1.4 and Table~\ref{tab:survey-comparison} is therefore essential to establishing the present survey's novelty.
...
We therefore organize the field around a single formal object, the \textbf{Bias Conduction Framework (BCF)}. BCF has two axes. The first axis is the \textbf{entry locus}: the agent component at which a demographic disparity first becomes measurable. The second axis is the \textbf{conduction operator}: how each downstream pipeline edge transforms a disparity that reaches it, attenuating it, preserving it, amplifying it, or generating a new one from clean input. A bias observation in this field is thus a coordinate pair (locus, operator), and a mitigation is an intervention on a specific coordinate. This is the dimension along which the near-neighbor works do \emph{not} organize: \cite{mohammadi2025evaluation} treats fairness as one sub-topic of a broad agent-evaluation survey; \cite{vatsal2026agentic} as a subdimension of a seven-dimensional conceptual taxonomy in a single (clinical) domain; \cite{mayilvaghanan2026counterfactual} as a single-domain counterfactual method (contact-center QA) whose CFR/MASD machinery we generalize across components in \S{}4; \cite{ranjan2025fairness} proposes a unified fairness framework for multi-agent AI broadly, without an LLM-component axis; and \cite{binkyte2025interactional} formalizes only the interactional slice of multi-agent fairness. None pairs an entry-locus decomposition with an account of how disparity \emph{travels}.
...
\item $T$ is the \emph{tool/environment interface}: the tool registry, retrieval indices, and external APIs; invoking $T$ returns an observation $o_t$ that is appended to the context. Tool-learning and reasoning-acting architectures formalize this interface \cite{schick2023toolformer,yao2023react,qu2024tool}.
...
\item $R$ is the \emph{feedback channel}: the loop-closing signal (user reactions within a dialogue, reward models, preference data, or retraining on the agent's own outputs) through which today's behavior conditions tomorrow's policy.
\end{itemize}

A \emph{trajectory} $\tau = (s_0, a_0, o_0, \ldots, a_T)$ terminates in a consequential output $O(\tau)$: a decision, ranking, allocation, or escalation. A \emph{multi-agent system} is a set $\{A_i\}$ plus a controller policy $\pi_{\mathrm{ctrl}}$ that assigns roles and routes messages \cite{wu2023autogen,guo2024large}. This tuple deliberately follows the standard agent anatomy of \S{}2.3 \cite{wang2024survey,xi2023rise,sumers2024cognitive}; BCF adds nothing to the architecture. It adds an accounting scheme over it.
...
\emph{Takeaway:} AMPLIFY and GENERATE are the two genuinely agentic modes. Single-turn evaluation cannot exhibit either, and (as Table~\ref{tab:component-harm} and \S{}7 show) GENERATE is the genuinely under-instrumented one, while AMPLIFY is empirically attested but rarely formalized in fairness terms.

GENERATE is formally a new entry rather than a transformation ($\varphi$ is undefined on zero input); we treat a GENERATE edge as contributing its own $\Delta_c$ term. ATTENUATE is also the operator class of every mitigation in \S{}5: a mitigation \emph{is} an engineered $\varphi$ < 1 edge, which is what makes the mitigation map of Table~\ref{tab:mitigation-map} commensurable with the taxonomy.

\paragraph{The conduction equation and the feedback recurrence}

Treating disparities as small and edges as locally multiplicative (a first-order approximation), and taking $\Delta_c$ and $\Delta_\tau$ here as \emph{signed} (directional) disparities so that oppositely directed component contributions can cancel in the sum (the endpoint statistics CFR and MASD of \S{}4.2 report the corresponding magnitudes |$\Delta$|), the trajectory disparity decomposes as the \textbf{conduction equation}:
...
\textbf{Definition.} Agents plan: they decompose a goal into sub-goals, order steps, and reason explicitly (chain-of-thought, ReAct traces \cite{yao2023react}) before acting. The deliberate-reasoning architectures that dominate this locus go beyond a single linear chain: Tree of Thoughts \cite{yao2023tot} explores and prunes a branching space of intermediate plans, and self-consistency \cite{wang2023selfconsistency} samples many reasoning paths and marginalizes over them by majority vote. Both are fairness-relevant precisely because they multiply and then select among decomposition steps: a demographic signal that tilts which branch is expanded or which sampled path wins the vote injects disparity into the plan even when each individual step looks unremarkable, and the selection mechanism can entrench rather than average out a skew. Planning bias is demographic disparity that enters during this reasoning/decomposition stage, in the \emph{plan}, not merely the final answer. By rule R2, allocation skew belongs here even when the allocation is over agent-flavored "team members."
...
\textbf{Evidence and synthesis.}

\emph{What is known.} The established, replicated mechanism is that deliberate reasoning is a GENERATE locus, not a sanitizing filter. The weight of evidence supports two related claims. First, explicit chain-of-thought reasoning changes social-bias outcomes relative to direct answering, and the change can be adverse: \cite{wu2025reasoning} demonstrates in controlled comparisons that chain-of-thought reasoning fails to deliver a fairness dividend and can worsen stereotype expression in ambiguous contexts, while reasoning-free models show more stable bias levels. Second, allocation-stage disparity is demonstrable at the plan object level rather than only at the final-output level: LLM-driven software-team composition produces demographically skewed role and task assignments, a planning-stage disparity with direct hiring adjacency \cite{parziale2026once}. Together these establish that the operator at C4 is GENERATE-consistent, pending the revealer-vs-generator adjudication \S{}3.4 itself describes. Unlike C3 (where interaction then amplifies what role-assignment generates), C4 disparity appears to resolve principally at the GENERATE edge itself, without requiring a subsequent amplification step, which makes it structurally more like C1 (early-entry, conducted forward) than like C3 (entry plus amplification). The lever exists: reasoning-guided fine-tuning reduces bias at this locus \cite{kabra2025reasoning}, an intervention consistent with P4 because it acts precisely at the generating edge; complementarily, a Fairness Reward Model can score complete reasoning chains against equalized opportunity and equalized odds criteria and reweight them during post-generation aggregation, leaving the internal reasoning untouched \cite{hall2025guiding}.

\emph{What conflicts or is contested.} Two tensions complicate the picture. First, the causal direction of the reasoning-bias relationship is not resolved. \cite{wu2025reasoning} shows reasoning can worsen bias, but the comparison baseline matters: the effect depends on whether direct answering suppresses or merely conceals the disparity, and those two mechanisms have sharply different implications for auditing. If direct answering conceals, then reasoning makes the bias \emph{visible} without increasing it, which would make the chain-of-thought trace a diagnostic asset rather than a liability. If reasoning genuinely generates disparity that a forward pass without explicit steps would not exhibit, then explicit planning is a fairness risk independent of the underlying model. The existing measurements do not cleanly separate these. Second, \cite{zhou2025veracity} documents hidden veracity-linked beliefs surfacing \emph{during} problem-solving, encoding demographic assumptions a final-answer probe would not detect. This result is consistent with both interpretations: reasoning as revealer or reasoning as generator. Until trace-replay (D2) is applied to hold the model's implicit beliefs constant across conditions and vary only the decomposition step, the two hypotheses remain confounded.

\emph{What transfers only by analogy.} The P2-masking problem at this locus depends critically on the finding that chain-of-thought explanations systematically misreport the biasing features that actually drive predictions \cite{turpin2023language}[$\dagger$]. This result originates in single-turn classification and explanation settings, not in instrumented agentic plan traces. The transfer argument is that the same model occupying $\pi$ in an agent produces unfaithful self-reports when generating rationales, as in the single-turn study, and therefore plan-trace audits at C4 inherit the same unreliability. That transfer is plausible but unmeasured: the degree to which multi-step plan generation is \emph{more} or \emph{less} faithful than single-step explanation is not known, and there is no agentic corpus study that validates the transfer directly. Similarly, the measurement reframing of \cite{li2025actions}[$\dagger$], showing that agent \emph{decisions} reveal implicit biases that text-level evaluation misses, provides the decisive motivation for plan-object auditing rather than trace-reading, but the original demonstration is in a simpler decision-making context rather than a full multi-step planning loop. The transferred conclusion (plan objects, not verbalized rationales, are the sound observables for C4) is well-motivated, but the agent-pipeline-specific validation has not been performed.

\emph{What is unmeasured.} The benchmark gap at this locus is the sharpest in the survey. No existing benchmark scores the \emph{plan itself} as a fairness object: are the sub-goals, their ordering, or the resource allocation across plan steps a counterfactual function of the subject's demographics, independent of the final decision? Plan-allocation disparity ($\Delta_{\mathrm{plan}}$, \S{}4.3) is observed in individual studies (\cite{wu2025reasoning,parziale2026once}) but has not been operationalized as a reusable metric with standardized demographic perturbations and counterfactual controls. The conduction signature below specifies what such a metric must measure. Critically, the P2-masking problem means that this gap cannot be closed by trace-reading: any future plan-audit must be counterfactual-behavioral (D2 applied to plan objects, holding the underlying evidence context fixed and swapping demographic signals in the goal specification), not inspection of verbalized rationales. That design constraint, established by \cite{turpin2023language}[$\dagger$]'s single-turn result and reinforced by \cite{li2025actions}[$\dagger$], has not yet been absorbed into a benchmark design. The result is that C4 sits at maturity $\bullet$$\circ$$\circ$ (Table~\ref{tab:component-harm}) despite documented real-world planning-stage harm in hiring-adjacent deployments: a measurement design problem, not a data availability problem.
...
The six entries are not additive, and Proposition P3 says when they can fail to be: \textbf{$\Delta_\tau$ can exceed the naive sum $\Sigma_c$ |$\Delta_c$| when at least one AMPLIFY or GENERATE edge lies on a conduction path.} P3 constitutes the formal content of what we call the \emph{agentic multiplier}, converting a rhetorical worry ("agents make bias worse") into a checkable property of a logged trace: estimate the $\varphi_e'$s; under the linearized magnitude model, some $\varphi_e$ > 1 or a fresh entry mid-path is necessary for super-additivity (not sufficient, since it can be offset downstream). Conversely (and this direction is just as consequential for auditors), if every edge in a pipeline is PRESERVE or ATTENUATE and no interaction term is present, component-level audits compose into a \emph{conservative} system-level bound, and certifying each component suffices. The entire practical question of whether per-component certification can ever underwrite system-level fairness claims therefore reduces to locating and bounding the amplifying edges.

\textbf{What is evidenced.} Within a single locus (C3) P3 is directly demonstrated. Stereotypical bias \emph{emerges} where none was salient, \emph{propagates} between agents, and is \emph{amplified} by their interaction, yielding a collective strictly more biased than its members \cite{nguyen2025social}; large-scale simulations show that fairness in multi-agent decision systems is \emph{emergent}, not reliably inherited, so component audits cannot be assumed to compose into a group-level guarantee in that setting \cite{madigan2025emergent}. Persona-induced bias scaling from single agents to societies \cite{li2025from} and interaction-borne implicit bias \cite{borah2024implicit} corroborate the same operator at the same locus. The temporal axis supplies a second evidenced amplifier, on the feedback edge rather than between components: self-consuming training loops with $\varphi_{\mathrm{fb}}$ > 1 \cite{wyllie2024fairness,wang2026observations,mansoury2020feedback}.
...
Here the trajectory disparity is larger than anything visible by summing component magnitudes, because an AMPLIFY (or GENERATE) edge on the conduction path multiplies the locus's contribution: the agentic multiplier of P3. We stress again that the 0.20, 0.24, 0.15, and $\varphi$ values are illustrative placeholders chosen to expose the operators, not estimates from any run.



\section{How to Evaluate Agent Fairness}

The translation from LLM-level fairness measurement to agent-level evaluation is neither direct nor complete. Single-turn benchmarks measure what a model \emph{says}; an agent must be evaluated on what the system \emph{does}: which tools it invokes, whose requests it escalates, how its plans allocate effort, and how disparities conduct across the loop. The Bias Conduction Framework of \S{}3 supplies the formal target: by Definition D2, fairness evaluation of an agent $\mathcal{A} = \langle \pi, T, M, R \rangle$ (Definition D1) is the estimation of component counterfactual disparities $\Delta_c$ via trace-replay, and by the conduction equation $\Delta_\tau \approx \sum_c \Delta_c \cdot \prod_{e} \varphi_e$, an endpoint-only measurement estimates the left-hand side while leaving every term on the right unidentified. This section delivers the survey's second contribution in four steps. \S{}4.1 lifts the classical metric families (group, individual, counterfactual) onto the agent loop and asks what the impossibility results of \cite{kleinberg2017inherent} and \cite{chouldechova2017fair} become when the unit of evaluation is a trajectory rather than a prediction. \S{}4.2 formalizes trajectory-level CFR and MASD, generalizing \cite{mayilvaghanan2026counterfactual} into instances of D2's $\Delta_c$. \S{}4.3 supplies formal definitions for the three action-level disparity metrics the field currently lacks (tool-invocation, escalation, and plan-allocation disparity) as $\Delta_c$ specializations, discharging Proposition P5's requirement constructively. \S{}4.4 then inventories the benchmark field (Table~\ref{tab:benchmarks}) and shows, via Propositions P2 and P5, why no existing resource can certify an acting agent.
...
\textbf{Tension T3 (aggregation masking, P2 as a measurement tension).} Proposition P2 states that endpoint parity is consistent with large offsetting component disparities (attenuating edges, $\varphi_e < 1$, or sign-opposed $\Delta_c$ terms). As a metric tension, this means group fairness at the trajectory endpoint and individual fairness along the trajectory can disagree without bound: counterfactual twins reach identical verdicts through unequally resourced paths. The unfaithfulness results for chain-of-thought sharpen the warning. \cite{turpin2023language} show that verbalized reasoning does not reliably reflect the causes of model behavior, so $\Delta_c$ must be defined over \emph{executed actions} (tool calls issued, memory operations performed, delegations made), never over the agent's narrated rationale.
...
classifying the edge as attenuating ($\hat{\varphi}_e < 1$), preserving ($\hat{\varphi}_e \approx 1$), or amplifying ($\hat{\varphi}_e > 1$); the GENERATE case is diagnosed when $\Delta_c \approx 0$ at every upstream locus yet $\Delta_{c'} > 0$. This turns the BCF's operator taxonomy from a descriptive vocabulary into a measurable quantity, and it is the estimator the audit protocol of \S{}6 is designed to compute (not estimated in the pilot; see \S{}6.5).
...
and the group form compares group-conditional mean invocation distributions, \begin{sloppypar}$\mathrm{TID}_{\mathrm{grp}} = \tfrac{1}{2} \sum_{t \in T} \big| \mathbb{E}[\, p_\tau(t) \mid a{=}1 \,] - \mathbb{E}[\, p_\tau(t) \mid a{=}0 \,] \big|$.\end{sloppypar} TID answers the question \S{}3.1 left open: does swapping a subject's demographics change \emph{which} API the agent calls, holding the query fixed? The existence evidence is layered: that selection skew is measurable and manipulable at all is established by the provider- and ordering-preference results of \cite{blankenstein2025biasbusters,sneh2025tooltweak}, which ride on the same surface-form sensitivities as option-order bias \cite{pezeshkpour2023large} but involve no sensitive attributes; the existence evidence for the demographic question itself rests on \cite{xu2026ducx}, where the tool stages account for an attributable share of total disparity in the one clinical agent where the decomposition has been run. No benchmark computes TID under demographic counterfactuals.
...
Three remarks complete the definitions. \emph{Estimation.} All three metrics are paired statistics under trace-replay: variance is computed over counterfactual pairs, confidence intervals attach to each $\Delta_c$, and the configuration-by-cue-channel grid requires multiple-comparison correction. The audit protocol of \S{}6 instantiates exactly these estimators for a hiring task across agent configurations C0 to C3 (the \S{}6.5 pilot reports endpoint metrics only; these paired/interior estimates await the full study). Determinism matters: without pinned versions, fixed decoding, and intercepted environment responses, flip rates confound demographic sensitivity with sampling noise. \emph{Scope.} The definitions deliberately score executed actions, not narrated rationales, per the unfaithfulness caveat of \cite{turpin2023language}. \emph{Adequacy.} Proposition P5 states that an evaluation suite is adequate for an agent only if it bounds $\Delta_c$ for every component with nonzero conduction weight in $\Delta_\tau \approx \sum_c \Delta_c \prod_e \varphi_e$. Definitions 4.3 to 4.5, together with the component CFR/MASD of \S{}4.2, constitute a minimal instrument set meeting P5 for the $T$, routing, and planning surfaces of D1. By Proposition P4 (mitigation-matching), they are also the precondition for crediting any mitigation with changing what an agent \emph{does} rather than what it says. What was previously described in this literature as having "no canonical formula" now has one; what remains missing is the benchmark infrastructure to run it against, which is \S{}4.4's subject.

\subsection{Datasets and Benchmarks: An Inventory of the Gap}

Table~\ref{tab:benchmarks} inventories the evaluation resources in the corpus alongside the agent-capability tools against which any claim of a "missing benchmark" must be tested. We classify each resource by \emph{fairness level} (QA (scored answers to fixed questions), generation (scored open-ended text), or action (scored executed behavior)) and by whether it is \emph{agentic}, which we define strictly: a resource is agentic if and only if it (i) executes multi-step, tool-using trajectories and (ii) scores a fairness quantity (some $\Delta_c$) over them under demographic instrumentation. Both conditions matter, and the table's structure shows the field has split them between two non-communicating literatures.
...
Multi-agent architectures introduce distinct bias pathways (\S{}3.3), but they also create structural opportunities for peer-level bias correction that single-agent systems cannot exploit. The multi-agent debiasing literature is the only mitigation family in Table~\ref{tab:mitigation-map} that acts natively at the agent-architecture level, operating on the delegation protocol and identity-signaling structure rather than on the base model's token distribution.
...
\textbf{Memory-level hygiene} addresses \S{}3.6's long-horizon drift by intervening on the agent's persistent state. \cite{ma2026implicit} characterizes how implicit biases accumulate across LLM long-term memory and proposes memory audit and selective forgetting mechanisms. Under the BCF, accumulated memory disparity is a feedback-recurrence path: each turn's $\Delta_c$ is written into memory and re-read as $\Delta_{\mathrm{entry}}$ for the next turn, creating the compounding effect P3 (Super-additivity) names. Memory hygiene (auditing which inferences are stored, which are forgotten) acts to reduce $\varphi_{\mathrm{fb}}$ in the recurrence equation, lowering the amplification factor over the trajectory. No corpus paper has demonstrated memory hygiene at scale in an agentic deployment, making this an architecturally motivated but empirically nascent intervention.
...
The taxonomy of \S{}3 argues that fairness in LLM-based agents is a property of pipeline components and their actions. The Bias Conduction Framework (BCF) formalizes this: P2 (Masking) holds that component-level disparity $\Delta_c$ may be invisible to metrics restricted to the output node; P3 (Super-additivity) holds that trajectory disparity $\Delta_\tau$ $\approx$ $\Sigma_c$ $\Delta_c$ $\cdot$ $\Pi$ $\varphi_e$ can exceed the sum of per-component disparities when conduction operators $\varphi_e$ > 1 along active pipeline edges. This section registers a \textbf{cross-framework bias audit} that serves as the BCF's primary falsification instrument: the same consequential-decision task is routed through four agent configurations (C0 to C3) spanning the BCF's conduction-operator space, and disparity is measured simultaneously at the answer level and at the action level.
...
\textbf{Profiles and perturbation.} All profiles are fully synthetic (N = 12 profiles: 6 strong and 6 borderline; no IRB required). The borderline tier is added deliberately to break the ceiling effect that, in an earlier N = 10 strong-only pilot, forced every candidate to advance and held the binary flip rate at zero. Base profiles hold qualifications fixed; the demographic signal is varied via candidate name in a 2x2 race-by-gender counterfactual that decouples the two attributes: White-male "Greg Becker", White-female "Emily Becker", Black-male "Jamal Washington", and Black-female "Lakisha Washington", following the name-audit lineage of \cite{nghiem2024you}, \cite{veldanda2023emily}, and \cite{pawar2025presumed}. Resume content is byte-for-byte identical across the four conditions; only the name changes. Each profile thus contributes four decisions per configuration, and disparity is read across the four name conditions. The data and harness are at \texttt{experiments/section6\_audit\_v2/}.
...
\textbf{H1: P2 Masking is real (the supported prediction).} Within a configuration, the continuous score-level disparity (MASD) is materially larger than the binary endpoint disparity (advance-rate disparity), so that the decision statistic understates the disparity carried in the score \cite{li2025actions}.
...
This study is deliberately modest in scale. It serves three purposes. First, it provides a \textbf{constructive existence proof} of P2 Masking and P3 Super-additivity as empirically measurable phenomena. Second, it establishes a \textbf{methodological template} (pinned models, synthetic counterfactuals, paired answer/action metrics, BCF-labeled configurations) that is portable across domains. Third, it serves as a \textbf{design seed} for the acting-agent benchmark called for in \S{}8.1. FairMedAgent, a forthcoming companion benchmark for clinical acting agents evaluated under matched demographic counterfactuals, is one scaled realization of this instrumentation; its development is independent of this survey's claims.

\subsection{Results: A Controlled Audit (audit v2)}

We report a controlled instantiation of the protocol that supersedes and replaces the earlier N = 10 strong-only pilot. The model under test is Claude Haiku 4.5 at temperature 0; the task is resume screening over the 12 synthetic profiles of \S{}6.1 (6 strong, 6 borderline); the counterfactual is the 2x2 race-by-gender name design (Greg Becker, Emily Becker, Jamal Washington, Lakisha Washington) over byte-identical resume content; and three configurations are run: C0 (single call), C2 (reason-then-decide), and C3 (a two-role recruiter-reviewer deliberation within one context). The design yields 144 decisions in total. Here CFR is the fraction of profiles whose advance/reject decision changes across the four name conditions; MASD is the mean across profiles of the max-minus-min score range over the four conditions on a 0-100 scale; the race gap is the mean score of White-coded minus Black-coded names (positive = White advantage) and the gender gap is male minus female (positive = male advantage); 95\% confidence intervals are bootstrap intervals from 3000 resamples over profiles. This audit instantiates the answer-level statistics (CFR, MASD) and the decision-action disparity; the tool-invocation and escalation metrics of \S{}6.2, which require a tool-using deployment, are left to the scaled companion benchmark.
...
Five observations follow, stated at the level the data support.

\emph{First, the decision-level disparity now registers.} With borderline candidates in the pool, the counterfactual flip rate reaches 0.33 in the borderline tier under C0 and C2, where the earlier strong-only pilot was ceiling-bound at zero. Adding borderline profiles was the design change that makes the binary endpoint informative at all.

\emph{Second, P2 (Masking) is supported.} In every configuration the score-level disparity (MASD of roughly 8 to 11 points) is substantially larger than the binary advance-rate disparity (0.08 to 0.17). A coarse advance/reject statistic understates the disparity that the underlying score carries at the same output node, which is exactly the endpoint-level masking P2 predicts.

\emph{Third, P3 / H2 (super-additivity, amplification with scaffold complexity) is not supported at this scale.} MASD is non-monotone across C0, C2, and C3 (8.8, 11.1, 8.2) with heavily overlapping confidence intervals, so no reliable ordering can be read off. This larger audit does not reproduce the earlier small pilot's apparent monotone MASD trend (1.2, 3.4, 6.4 across C0, C2, C3). We state this plainly: the pilot's apparent trend was most likely an over-reading of noise in a single small run, and the contrast is a cautionary illustration of exactly the measurement-rigor thesis this survey argues for.

\emph{Fourth, a small score skew favoring Black-coded names appears in the direct configuration.} Under C0 the race gap is -3.8 with a 95\% CI of [-6.7, -1.4] that excludes zero, indicating a modest advantage for Black-coded names; this attenuates to non-significance under explicit reasoning (C2 race gap +1.5, n.s.). Gender effects are not significant in any configuration. Pro-minority and pro-female skews of this kind are consistent with prior LLM-hiring audits \cite{an2025measuring}.

\emph{Fifth, scope caveats remain.} The audit is still modest in scale: N = 12 profiles, one name set per cell, one model (Haiku 4.5 only), and one task. The names encode race and gender jointly within each cell, so the 2x2 separates the two axes only at the margin level, not within a single label. This is a controlled measurement, not a definitive audit. The scaled instrument is the forthcoming companion benchmark already referenced in \S{}6.4 and \S{}8.1. As before, the C2 configuration covers only the reasoning edge and the C3 configuration is a single-context recruiter-reviewer proxy rather than a true two-agent edge with a $\pi_{\mathrm{ctrl}}$ controller; a full two-agent configuration with tool-using and escalation channels is future work. The harness, synthetic profiles, and raw decisions are released at \texttt{experiments/section6\_audit\_v2/}.


\section{Coverage Map and Cross-Analysis}

This section projects the 202-paper corpus onto its two organizing axes simultaneously: the six agent-pipeline components and the four fairness dimensions (group, individual, counterfactual, and unspecified). The projection is Figure~\ref{fig:coverage} (the coverage matrix) and its companion, Figure~\ref{fig:operator} (the locus $\times$ BCF-operator grid). Read together, they convert the survey's central claim (that the field is well-instrumented where measurement is easy and under-instrumented where agentic deployment makes fairness consequential) from an assertion into a structured, falsifiable coordinate.

\textbf{Tagging convention.} The corpus comprises \textbf{202 unique papers}. Of these, \textbf{59 papers} are tagged to at least one of the six agent-pipeline component branches in \texttt{corpus\_master.md}. Because a single paper may address more than one component (e.g., a study of RAG-induced bias in a multi-agent pipeline is tagged to both memory-retrieval and multi-agent delegation), the 59 unique component-tagged papers generate \textbf{68 component-branch entries} across the coverage matrix, more entries than papers. All counts in the narrative below follow this multi-tag convention. The remaining 143 papers (foundations, llm-agents-general, evaluation-metrics-benchmarks, mitigation, high-stakes-domains, and prior-art-additions branches) are covered in \S{}\S{}2-5 as contextual and cross-cutting evidence; they inform the taxonomy but do not appear in the six-component rows of Figure~\ref{fig:coverage}.

\subsection{The Counterfactual Column is Essentially Empty}

The headline of Figure~\ref{fig:coverage} is a near-vacant column. Across all six component rows, formal counterfactual fairness is the named measurement framing in \textbf{exactly one paper}: \cite{she2025fairsense}, under long-horizon drift, which simulates demographic-conditioned ML-system trajectories over time. The remaining five components (tool/API selection, memory and retrieval, multi-agent delegation, planning/decomposition, and user modeling/personalization) each register \textbf{zero} counterfactual entries.
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  % --- cells: \bcfcell{col}{gridy}{count}{max}{w}{h}, max=12 ---
  % Tool/API (gridy 5)
...
This is a structural inversion of need. The counterfactual flip (re-running the same agent on a profile altered only in a protected attribute and asking whether the trajectory changed) is the natural operationalization of the agentic fairness question: "would the agent have \emph{acted} differently for another demographic?" Group-level metrics, computed over output distributions, cannot directly answer this. Counterfactual flip can. In BCF terms, the counterfactual swap is the procedure that produces D2's $\Delta_c$, the component counterfactual disparity that enters the conduction equation. The dimension that BCF-based auditing most requires is precisely the dimension the field has least applied. The point is not weakened by the lone exemplar: even that single counterfactual entry, \cite{she2025fairsense}, is itself adjacent and non-agentic (marked [$\dagger$] in \S{}3.6, a generic ML-system drift simulator never applied to an instrumented LLM-agent trajectory), so the counterfactual column is empty of properly agentic counterfactual auditing rather than holding a genuine exception.
...
Third, \textbf{the modal corpus paper documents disparity without naming a fairness dimension or a conduction operator}: 52 of the 68 component-branch entries sit in the unspecified column of Figure~\ref{fig:coverage}, and most of the locus $\times$ operator cells in Figure~\ref{fig:operator} are under-populated. The field is evidence-rich and formalization-poor.

Fourth, the BCF coordinate system makes the gaps precise and actionable. The empty cells of Figure~\ref{fig:operator} are specific (component, operator) pairs that can be populated by targeted experiments with the counterfactual-perturbation machinery already available \cite{parziale2025systematic,mayilvaghanan2026counterfactual,basu2026names}.

This analysis also sharpens the differentiation from neighboring work (Table~\ref{tab:survey-comparison} in \S{}1.4). Where \cite{chu2024fairness} organizes single-turn LLM fairness by metric and \cite{gallegos2024bias} by datasets and mitigation, neither resolves \emph{where in an acting pipeline} a disparity originates. Where \cite{mohammadi2025evaluation} treats fairness as one sub-topic of agent evaluation and \cite{vatsal2026agentic} as a subdimension of a seven-dimensional clinical taxonomy, neither makes component coverage its unit of analysis. Where \cite{ranjan2025fairness} proposes a framework for fairness in multi-agent AI broadly, it does not operate at the LLM-agent component level or develop the conduction-operator analysis. By projecting the corpus onto the component $\times$ operator grid, this survey turns the measurement gap from an assertion into a structured coordinate (the empty counterfactual column and the under-populated GENERATE/AMPLIFY cells) and hands \S{}8 a concrete, BCF-keyed research target.


\section{Open Problems and Research Agenda}

The coverage analysis of \S{}7 converts the measurement gap from an observation into a set of precise coordinates: (component, operator) cells in Figure~\ref{fig:operator} that are empirically uncharacterized, P5 Measurement-adequacy cells that no existing benchmark populates, and BCF propositions whose empirical grounding is weak. Each problem below opens with its BCF coordinate, states why it matters for consequential deployment, and specifies a concrete first step (a metric definition, a dataset structure, or a system design) that an immediate follow-on could execute.
...
\textbf{Concrete first step.} Draft a \textbf{minimum audit specification} for acting agents with four components: (i) a protocol for constructing counterfactual profile sets (building on \cite{parziale2025systematic}); (ii) a required metric set covering, at minimum, per-component CFR, tool-invocation disparity, and escalation-rate disparity, aligned with the BCF's D2 definition; (iii) a standardized reporting template with per-component $\Delta_c$ values and the conduction-operator classification (ATTENUATE/PRESERVE/AMPLIFY/GENERATE) for each active edge; and (iv) a recourse specification covering what information must be logged to enable post-hoc attribution and contestation. Component (iv) is more than a logging requirement: per-component $\Delta_c$ attribution is a natural substrate for algorithmic recourse and contestability \cite{karimi2022recourse}, because localizing a disparity to a specific pipeline edge (the tool that was not offered, the retrieval that was skewed, the plan step that was cut) tells an affected person what to contest and tells the operator which edge to change, which a single endpoint-level parity number cannot. This specification can be drafted as a technical standard proposal and submitted for comment to the NIST AI 100-1 framework revision process.

\subsection{Mitigation at the Agent Level, Not the Token Level}

\textbf{BCF coordinate.} P4 Mitigation-matching states that an intervention is matched if it operates at the same pipeline locus as the disparity it is meant to correct. The mitigation map in \S{}5 shows that the ATTENUATE column of Figure~\ref{fig:operator} is populated almost entirely by token-level interventions (prompting, alignment, output filtering), which are mismatched to the six agent-pipeline loci where the AMPLIFY and GENERATE operators are documented.
...
The cross-framework audit of \S{}6 is reported as a controlled but modest-scale study (\S{}6.5): N = 12 synthetic profiles (6 strong, 6 borderline), a decoupled 2x2 race-by-gender name counterfactual, 144 decisions, and bootstrap 95\% confidence intervals over 3000 profile resamples. The borderline tier addresses the decision-level ceiling effect that bound the earlier strong-only pilot to CFR = 0. As a result:

\begin{itemize}
\item P2 (Masking) is supported by the audit: the score-level disparity (MASD of roughly 8 to 11 points) is several times the binary advance-rate disparity (0.08 to 0.17) in every configuration. P3 / H2 (amplification with scaffold complexity) is not supported at this scale: MASD is non-monotone across C0, C2, and C3 with overlapping confidence intervals, and the audit does not reproduce the earlier small pilot's apparent monotone MASD trend. Readers should treat P2 as supported by this controlled run together with the multi-paper evidence cited throughout \S{}\S{}3-4, and P3's cross-component amplification form as a well-motivated conjecture that this audit does not confirm.
\item The audit remains modest in scale (one model, Haiku 4.5; one task; one name set per cell; names that encode race and gender jointly within each label), so it is a controlled measurement rather than a definitive audit. The stance is non-confirmatory by design.
...
These limitations are by design: the survey's contribution is the framework, the protocol, and a controlled first measurement, not a definitive empirical study. The harness and data are released as a replication target; the scaled benchmark is the immediate next step (\S{}8.1).
...
Readers should treat the coverage matrix as a snapshot of the field as of mid-2026, not as a permanent characterization. The BCF framework and the open-problem agenda of \S{}8 are designed to remain relevant as the empirical field changes; the specific cell counts in Figure~\ref{fig:coverage} and Figure~\ref{fig:operator} should be updated as new work appears.

%% === BIBLIOGRAPHY INLINED (was: \bibliographystyle{ACM-Reference-Format} + \bibliography{references}) ===
%% Source: main.bbl (ACM-Reference-Format, 168 bibitems). main.tex now compiles with pdflatex alone.
%%% -*-BibTeX-*-
...
\bibitem[Barocas and Narayanan(2023)]%
        {barocas2023fairness}
\bibfield{author}{\bibinfo{person}{Hardt Barocas} {and}
  \bibinfo{person}{Narayanan}.} \bibinfo{year}{2023}\natexlab{}.
\newblock \showarticletitle{Fairness and Machine Learning: Limitations and
  Opportunities}.
...
\bibitem[Binkyte(2025)]%
        {binkyte2025interactional}
\bibfield{author}{\bibinfo{person}{Ruta Binkyte}.}
  \bibinfo{year}{2025}\natexlab{}.
\newblock \showarticletitle{Interactional Fairness in LLM Multi-Agent Systems:
  An Evaluation Framework}.
\newblock  (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2505.12001}
\href{https://doi.org/10.1609/aies.v8i1.36563}{doi:\nolinkurl{10.1609/aies.v8i1.36563}}

\bibitem[Borah and Mihalcea(2024)]%
        {borah2024implicit}
\bibfield{author}{\bibinfo{person}{Borah} {and} \bibinfo{person}{Mihalcea}.}
  \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{Towards Implicit Bias Detection and Mitigation in
  Multi-Agent LLM Interactions}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2410.02584}
\urldef\tempurl%
\url{https://arxiv.org/abs/2410.02584}
\showURL{%
\tempurl}

\bibitem[Bouchard(2024)]%
        {bouchard2024bring}
\bibfield{author}{\bibinfo{person}{D. Bouchard}.}
  \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{Bring Your Own Prompts: Use-Case-Specific Bias and
  Fairness Evaluation for LLMs (LangFair)}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2407.10853}
\urldef\tempurl%
\url{https://arxiv.org/abs/2407.10853}
\showURL{%
\tempurl}

\bibitem[Buolamwini and Gebru(2018)]%
        {buolamwini2018gender}
\bibfield{author}{\bibinfo{person}{Buolamwini} {and} \bibinfo{person}{Gebru}.}
  \bibinfo{year}{2018}\natexlab{}.
\newblock \showarticletitle{Gender Shades: Intersectional Accuracy Disparities
...
\bibitem[Caton and Haas(2024)]%
        {caton2024fairness}
\bibfield{author}{\bibinfo{person}{Simon Caton} {and}
  \bibinfo{person}{Christian Haas}.} \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{Fairness in Machine Learning: A Survey}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2010.04053}
\href{https://doi.org/10.1145/3616865}{doi:\nolinkurl{10.1145/3616865}}

\bibitem[Chang et~al\mbox{.}(2024)]%
        {chang2024survey}
\bibfield{author}{\bibinfo{person}{Yupeng Chang}, \bibinfo{person}{Xu Wang},
  \bibinfo{person}{Jindong Wang}, \bibinfo{person}{Yuan Wu},
  \bibinfo{person}{Linyi Yang}, \bibinfo{person}{Kaijie Zhu},
  \bibinfo{person}{Hao Chen}, \bibinfo{person}{Xiaoyuan Yi},
  \bibinfo{person}{Cunxiang Wang}, \bibinfo{person}{Yidong Wang},
  \bibinfo{person}{Wei Ye}, \bibinfo{person}{Yue Zhang}, \bibinfo{person}{Yi
  Chang}, \bibinfo{person}{Philip~S. Yu}, \bibinfo{person}{Qiang Yang}, {and}
  \bibinfo{person}{Xing Xie}.} \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{A Survey on Evaluation of Large Language Models}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2307.03109}
\href{https://doi.org/10.1145/3641289}{doi:\nolinkurl{10.1145/3641289}}

\bibitem[Chouldechova(2017)]%
        {chouldechova2017fair}
\bibfield{author}{\bibinfo{person}{Chouldechova}.}
  \bibinfo{year}{2017}\natexlab{}.
\newblock \showarticletitle{Fair Prediction with Disparate Impact: A Study of
  Bias in Recidivism Prediction Instruments}.
\newblock  (\bibinfo{year}{2017}).
\newblock
\showeprint[arxiv]{1703.00056}
\href{https://doi.org/10.1089/big.2016.0047}{doi:\nolinkurl{10.1089/big.2016.0047}}

\bibitem[Dai et~al\mbox{.}(2025)]%
        {dai2025trustworthy}
\bibfield{author}{\bibinfo{person}{Sunhao Dai}, \bibinfo{person}{Changle Qu},
  \bibinfo{person}{Sirui Chen}, \bibinfo{person}{Xiao Zhang}, {and}
  \bibinfo{person}{Jun Xu}.} \bibinfo{year}{2025}\natexlab{}.
\newblock \showarticletitle{Trustworthy Information Retrieval in the LLM Era:
  Bias, Unfairness, and Hallucination}.
\newblock  (\bibinfo{year}{2025}).
\newblock
\href{https://doi.org/10.1145/3767695.3769670}{doi:\nolinkurl{10.1145/3767695.3769670}}

\bibitem[Ebrahimi and Asudeh(2025)]%
        {ebrahimi2025survey}
\bibfield{author}{\bibinfo{person}{Sana Ebrahimi} {and}
  \bibinfo{person}{Abolfazl Asudeh}.} \bibinfo{year}{2025}\natexlab{}.
\newblock \showarticletitle{A Survey on Reliability, Transparency,
  Accountability, and Fairness in LLM-based Multi-Agent Systems through the
  Responsibility Lens}.
\newblock \bibinfo{journal}{\emph{Preprint (ResearchGate/unpublished),
  University of Illinois Chicago}} (\bibinfo{year}{2025}).
\newblock

\bibitem[et~al.(2024a)]%
        {allam2024biasdpo}
\bibfield{author}{\bibinfo{person}{Allam et al.}}
  \bibinfo{year}{2024}\natexlab{a}.
\newblock \showarticletitle{BiasDPO: Mitigating Bias in Language Models through
  Direct Preference Optimization}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2407.13928}
\urldef\tempurl%
...
\bibitem[et~al.(2024b)]%
        {arzaghi2024understanding}
\bibfield{author}{\bibinfo{person}{Arzaghi et al.}}
  \bibinfo{year}{2024}\natexlab{b}.
\newblock \showarticletitle{Understanding Intrinsic Socioeconomic Biases in
  Large Language Models}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2405.18662}
\urldef\tempurl%
...
\bibitem[et~al.(2025a)]%
        {azime2025accept}
\bibfield{author}{\bibinfo{person}{Azime et al.}}
  \bibinfo{year}{2025}\natexlab{a}.
\newblock \showarticletitle{Accept or Deny? Evaluating LLM Fairness and
  Performance in Loan Approval across Table-to-Text Serialization Approaches}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2508.21512}
\urldef\tempurl%
...
\bibitem[et~al.(2025b)]%
        {an2025measuring}
\bibfield{author}{\bibinfo{person}{An et al.}}
  \bibinfo{year}{2025}\natexlab{b}.
\newblock \showarticletitle{Measuring gender and racial biases in large
  language models: Intersectional evidence from automated resume evaluation}.
\newblock  (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2403.15281}
\href{https://doi.org/10.1093/pnasnexus/pgaf089}{doi:\nolinkurl{10.1093/pnasnexus/pgaf089}}

\bibitem[et~al.(2025c)]%
        {adappanavar2025mfarm}
\bibfield{author}{\bibinfo{person}{Adappanavar et al.}}
  \bibinfo{year}{2025}\natexlab{c}.
\newblock \showarticletitle{mFARM: Towards Multi-Faceted Fairness Assessment
  based on HARMs in Clinical Decision Support}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2509.02007}
\urldef\tempurl%
...
\bibitem[et~al.(2026a)]%
        {amirimargavi2026equal}
\bibfield{author}{\bibinfo{person}{Amiri-Margavi et al.}}
  \bibinfo{year}{2026}\natexlab{a}.
\newblock \showarticletitle{Equal Access, Unequal Interaction: A Counterfactual
  Audit of LLM Fairness}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2026}).
\newblock
\showeprint[arxiv]{2602.02932}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Blodgett et al.}}
  \bibinfo{year}{2020}\natexlab{a}.
\newblock \showarticletitle{Language (Technology) is Power: A Critical Survey
  of Bias in NLP}.
\newblock  (\bibinfo{year}{2020}).
\newblock
\showeprint[arxiv]{2005.14050}

\bibitem[et~al.(2021a)]%
        {blodgett2021stereotyping}
\bibfield{author}{\bibinfo{person}{Blodgett et al.}}
  \bibinfo{year}{2021}\natexlab{a}.
\newblock \showarticletitle{Stereotyping Norwegian Salmon: An Inventory of
  Pitfalls in Fairness Benchmark Datasets}.
\newblock  (\bibinfo{year}{2021}).
\newblock
\href{https://doi.org/10.18653/v1/2021.acl-long.81}{doi:\nolinkurl{10.18653/v1/2021.acl-long.81}}

\bibitem[et~al.(2022a)]%
        {bai2022constitutional}
\bibfield{author}{\bibinfo{person}{Bai et al.}}
  \bibinfo{year}{2022}\natexlab{a}.
\newblock \showarticletitle{Constitutional AI: Harmlessness from AI Feedback}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2022}).
\newblock
\showeprint[arxiv]{2212.08073}
\urldef\tempurl%
...
\bibitem[et~al.(2024c)]%
        {bowen2024measuring}
\bibfield{author}{\bibinfo{person}{Bowen et al.}}
  \bibinfo{year}{2024}\natexlab{c}.
\newblock \showarticletitle{Measuring and Mitigating Racial Disparities in
  LLMs: Evidence from a Mortgage Underwriting Experiment}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\href{https://doi.org/10.2139/ssrn.4812158}{doi:\nolinkurl{10.2139/ssrn.4812158}}

\bibitem[et~al.(2025d)]%
        {blankenstein2025biasbusters}
\bibfield{author}{\bibinfo{person}{Blankenstein et al.}}
  \bibinfo{year}{2025}\natexlab{d}.
\newblock \showarticletitle{BiasBusters: Uncovering and Mitigating Tool
  Selection Bias in Large Language Models}.
\newblock \bibinfo{journal}{\emph{ArXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2510.00307}
\urldef\tempurl%
...
\bibitem[et~al.(2026b)]%
        {basu2026names}
\bibfield{author}{\bibinfo{person}{Basu et al.}}
  \bibinfo{year}{2026}\natexlab{b}.
\newblock \showarticletitle{When Names Change Verdicts: Intervention
  Consistency Reveals Systematic Bias in LLM Decision-Making}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2026}).
\newblock
\showeprint[arxiv]{2603.18530}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Caliskan et al.}}
  \bibinfo{year}{2017}\natexlab{a}.
\newblock \showarticletitle{Semantics Derived Automatically from Language
  Corpora Contain Human-Like Biases}.
\newblock  (\bibinfo{year}{2017}).
\newblock
\showeprint[arxiv]{1608.07187}
\href{https://doi.org/10.1126/science.aal4230}{doi:\nolinkurl{10.1126/science.aal4230}}

\bibitem[et~al.(2024d)]%
        {chu2024fairness}
\bibfield{author}{\bibinfo{person}{Chu et al.}}
  \bibinfo{year}{2024}\natexlab{d}.
\newblock \showarticletitle{Fairness in Large Language Models: A Taxonomic
  Survey}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2404.01349}
\href{https://doi.org/10.1145/3682112.3682117}{doi:\nolinkurl{10.1145/3682112.3682117}}

\bibitem[et~al.(2024e)]%
        {campedelli2024want}
\bibfield{author}{\bibinfo{person}{Campedelli et al.}}
  \bibinfo{year}{2024}\natexlab{e}.
\newblock \showarticletitle{I Want to Break Free! Persuasion and Anti-Social
  Behavior of LLMs in Multi-Agent Settings with Social Hierarchy}.
\newblock \bibinfo{journal}{\emph{arXiv preprint (EACL 2024 Workshop)}}
  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2410.07109}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Chakraborty et al.}}
  \bibinfo{year}{2024}\natexlab{f}.
\newblock \showarticletitle{MaxMin-RLHF: Alignment with Diverse Human
  Preferences}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2402.08925}
\urldef\tempurl%
...
\bibitem[et~al.(2025e)]%
        {coppolillo2025unmasking}
\bibfield{author}{\bibinfo{person}{Coppolillo et al.}}
  \bibinfo{year}{2025}\natexlab{e}.
\newblock \showarticletitle{Unmasking Conversational Bias in AI Multiagent
  Systems}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2501.14844}
\urldef\tempurl%
...
\bibitem[et~al.(2025f)]%
        {choi2025identity}
\bibfield{author}{\bibinfo{person}{Choi et al.}}
  \bibinfo{year}{2025}\natexlab{f}.
\newblock \showarticletitle{When Identity Skews Debate: Anonymization for
  Bias-Reduced Multi-Agent Reasoning}.
\newblock  (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2510.07517}
\urldef\tempurl%
...
\bibitem[et~al.(2026c)]%
        {cao2026from}
\bibfield{author}{\bibinfo{person}{Cao et al.}}
  \bibinfo{year}{2026}\natexlab{c}.
\newblock \showarticletitle{From Biased Chatbots to Biased Agents: Examining
  Role Assignment Effects on LLM Agent Robustness}.
\newblock  (\bibinfo{year}{2026}).
\newblock
\showeprint[arxiv]{2602.12285}
\urldef\tempurl%
\url{https://arxiv.org/abs/2602.12285}
\showURL{%
\tempurl}

\bibitem[Crenshaw(1989)]%
        {crenshaw1989demarginalizing}
\bibfield{author}{\bibinfo{person}{Kimberl\'e Crenshaw}.}
  \bibinfo{year}{1989}\natexlab{}.
\newblock \showarticletitle{Demarginalizing the Intersection of Race and Sex: A
  Black Feminist Critique of Antidiscrimination Doctrine, Feminist Theory and
  Antiracist Politics}.
\newblock \bibinfo{journal}{\emph{University of Chicago Legal Forum}}
  \bibinfo{volume}{1989} (\bibinfo{year}{1989}),
  \bibinfo{pages}{139 to 167}.

\bibitem[et~al.(2012)]%
        {dwork2012fairness}
\bibfield{author}{\bibinfo{person}{Dwork et al.}}
  \bibinfo{year}{2012}\natexlab{}.
\newblock \showarticletitle{Fairness Through Awareness}.
\newblock  (\bibinfo{year}{2012}).
\newblock
\showeprint[arxiv]{1104.3913}
\href{https://doi.org/10.1145/2090236.2090255}{doi:\nolinkurl{10.1145/2090236.2090255}}

\bibitem[et~al.(2021b)]%
        {dhamala2021bold}
\bibfield{author}{\bibinfo{person}{Dhamala et al.}}
  \bibinfo{year}{2021}\natexlab{b}.
\newblock \showarticletitle{BOLD: Dataset and Metrics for Measuring Biases in
  Open-Ended Language Generation}.
\newblock  (\bibinfo{year}{2021}).
\newblock
\showeprint[arxiv]{2101.11718}
\href{https://doi.org/10.1145/3442188.3445924}{doi:\nolinkurl{10.1145/3442188.3445924}}

\bibitem[et~al.(2023a)]%
        {deng2023mind2web}
\bibfield{author}{\bibinfo{person}{Deng et al.}}
  \bibinfo{year}{2023}\natexlab{a}.
\newblock \showarticletitle{Mind2Web: Towards a Generalist Agent for the Web}.
\newblock  (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2306.06070}

\bibitem[et~al.(2023b)]%
        {deshpande2023toxicity}
\bibfield{author}{\bibinfo{person}{Deshpande et al.}}
  \bibinfo{year}{2023}\natexlab{b}.
\newblock \showarticletitle{Toxicity in ChatGPT: Analyzing Persona-assigned
  Language Models}.
\newblock  (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2304.05335}

\bibitem[et~al.(2024g)]%
        {durante2024agent}
\bibfield{author}{\bibinfo{person}{Durante et al.}}
  \bibinfo{year}{2024}\natexlab{g}.
\newblock \showarticletitle{Agent AI: Surveying the Horizons of Multimodal
  Interaction}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2401.03568}

\bibitem[et~al.(2024h)]%
        {dai2024bias}
\bibfield{author}{\bibinfo{person}{Dai et al.}}
  \bibinfo{year}{2024}\natexlab{h}.
\newblock \showarticletitle{Bias and Unfairness in Information Retrieval
  Systems: New Challenges in the LLM Era}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2404.11457}
\href{https://doi.org/10.1145/3637528.3671458}{doi:\nolinkurl{10.1145/3637528.3671458}}

\bibitem[et~al.(2024i)]%
        {dong2024building}
\bibfield{author}{\bibinfo{person}{Dong et al.}}
  \bibinfo{year}{2024}\natexlab{i}.
\newblock \showarticletitle{Building Guardrails for Large Language Models}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2402.01822}
\urldef\tempurl%
...
\bibitem[et~al.(2024j)]%
        {du2024causal}
\bibfield{author}{\bibinfo{person}{Du et al.}}
  \bibinfo{year}{2024}\natexlab{j}.
\newblock \showarticletitle{Causal-Guided Active Learning for Debiasing Large
  Language Models}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2408.12942}
\urldef\tempurl%
...
\bibitem[et~al.(2024k)]%
        {deldjoo2024cfairllm}
\bibfield{author}{\bibinfo{person}{Deldjoo et al.}}
  \bibinfo{year}{2024}\natexlab{k}.
\newblock \showarticletitle{CFaiRLLM: Consumer Fairness Evaluation in
  Large-Language Model Recommender System}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2403.05668}
\urldef\tempurl%
...
\bibitem[et~al.(2024l)]%
        {deldjoo2024normative}
\bibfield{author}{\bibinfo{person}{Deldjoo et al.}}
  \bibinfo{year}{2024}\natexlab{l}.
\newblock \showarticletitle{A Normative Framework for Benchmarking Consumer
  Fairness in Large Language Model Recommender Systems}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2405.02219}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Feldman et al.}}
  \bibinfo{year}{2015}\natexlab{}.
\newblock \showarticletitle{Certifying and Removing Disparate Impact}.
\newblock  (\bibinfo{year}{2015}).
\newblock
\showeprint[arxiv]{1412.3756}
\href{https://doi.org/10.1145/2783258.2783311}{doi:\nolinkurl{10.1145/2783258.2783311}}

\bibitem[et~al.(2023c)]%
        {feng2023empowering}
\bibfield{author}{\bibinfo{person}{Feng et al.}}
  \bibinfo{year}{2023}\natexlab{c}.
\newblock \showarticletitle{Empowering Many, Biasing a Few: Generalist Credit
  Scoring through Large Language Models}.
\newblock \bibinfo{journal}{\emph{arXiv}} (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2310.00566}
\urldef\tempurl%
...
\bibitem[et~al.(2024m)]%
        {fan2024fairmt}
\bibfield{author}{\bibinfo{person}{Fan et al.}}
  \bibinfo{year}{2024}\natexlab{m}.
\newblock \showarticletitle{FairMT-Bench: Benchmarking Fairness for Multi-turn
  Dialogue in Conversational LLMs}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2410.19317}
\urldef\tempurl%
...
\bibitem[et~al.(2024n)]%
        {furniturewala2024thinking}
\bibfield{author}{\bibinfo{person}{Furniturewala et al.}}
  \bibinfo{year}{2024}\natexlab{n}.
\newblock \showarticletitle{Thinking Fair and Slow: On the Efficacy of
  Structured Prompts for Debiasing Language Models}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2405.10431}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Gehman et al.}}
  \bibinfo{year}{2020}\natexlab{b}.
\newblock \showarticletitle{RealToxicityPrompts: Evaluating Neural Toxic
  Degeneration in Language Models}.
\newblock  (\bibinfo{year}{2020}).
\newblock
\showeprint[arxiv]{2009.11462}

\bibitem[et~al.(2023d)]%
        {gupta2023bias}
\bibfield{author}{\bibinfo{person}{Gupta et al.}}
  \bibinfo{year}{2023}\natexlab{d}.
\newblock \showarticletitle{Bias Runs Deep: Implicit Reasoning Biases in
  Persona-Assigned LLMs}.
\newblock  (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2311.04892}
\urldef\tempurl%
\url{https://arxiv.org/abs/2311.04892}
\showURL{%
\tempurl}

\bibitem[Foulds et~al\mbox{.}(2020)]%
        {foulds2020intersectional}
\bibfield{author}{\bibinfo{person}{James~R. Foulds}, \bibinfo{person}{Rashidul
  Islam}, \bibinfo{person}{Kamrun~Naher Keya}, {and} \bibinfo{person}{Shimei
  Pan}.} \bibinfo{year}{2020}\natexlab{}.
\newblock \showarticletitle{An Intersectional Definition of Fairness}.
\newblock \bibinfo{journal}{\emph{2020 IEEE 36th International Conference on Data
  Engineering (ICDE)}} (\bibinfo{year}{2020}).
\newblock
\showeprint[arxiv]{1807.08362}

\bibitem[et~al.(2024o)]%
        {gallegos2024bias}
\bibfield{author}{\bibinfo{person}{Gallegos et al.}}
  \bibinfo{year}{2024}\natexlab{o}.
\newblock \showarticletitle{Bias and Fairness in Large Language Models: A
  Survey}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2309.00770}
\urldef\tempurl%
...
\bibitem[et~al.(2024p)]%
        {guo2024large}
\bibfield{author}{\bibinfo{person}{Guo et al.}}
  \bibinfo{year}{2024}\natexlab{p}.
\newblock \showarticletitle{Large Language Model based Multi-Agents: A Survey
  of Progress and Challenges}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2402.01680}

\bibitem[et~al.(2025g)]%
        {geng2025accumulating}
\bibfield{author}{\bibinfo{person}{Geng et al.}}
  \bibinfo{year}{2025}\natexlab{g}.
\newblock \showarticletitle{Accumulating Context Changes the Beliefs of
  Language Models}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2511.01805}
\urldef\tempurl%
...
\bibitem[et~al.(2025h)]%
        {ghosh2025medequalqa}
\bibfield{author}{\bibinfo{person}{Ghosh et al.}}
  \bibinfo{year}{2025}\natexlab{h}.
\newblock \showarticletitle{MEDEQUALQA: Evaluating Biases in LLMs with
  Counterfactual Reasoning}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2510.12818}
\urldef\tempurl%
...
\bibitem[et~al.(2016)]%
        {hardt2016equality}
\bibfield{author}{\bibinfo{person}{Hardt et al.}}
  \bibinfo{year}{2016}\natexlab{}.
\newblock \showarticletitle{Equality of Opportunity in Supervised Learning}.
\newblock  (\bibinfo{year}{2016}).
\newblock
\showeprint[arxiv]{1610.02413}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Hartvigsen et al.}}
  \bibinfo{year}{2022}\natexlab{b}.
\newblock \showarticletitle{ToxiGen: A Large-Scale Machine-Generated Dataset
  for Adversarial and Implicit Hate Speech Detection}.
\newblock  (\bibinfo{year}{2022}).
\newblock
\showeprint[arxiv]{2203.09509}

\bibitem[et~al.(2023e)]%
        {huang2023trustgpt}
\bibfield{author}{\bibinfo{person}{Huang et al.}}
  \bibinfo{year}{2023}\natexlab{e}.
\newblock \showarticletitle{TrustGPT: A Benchmark for Trustworthy and
  Responsible Large Language Models}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2306.11507}

\bibitem[et~al.(2024q)]%
        {huang2024collective}
\bibfield{author}{\bibinfo{person}{Huang et al.}}
  \bibinfo{year}{2024}\natexlab{q}.
\newblock \showarticletitle{Collective Constitutional AI: Aligning a Language
  Model with Public Input}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2406.07814}
\urldef\tempurl%
...
\bibitem[et~al.(2024r)]%
        {hong2024metagpt}
\bibfield{author}{\bibinfo{person}{Hong et al.}}
  \bibinfo{year}{2024}\natexlab{r}.
\newblock \showarticletitle{MetaGPT: Meta Programming for A Multi-Agent
  Collaborative Framework}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2308.00352}

\bibitem[et~al.(2024s)]%
        {hu2024free}
\bibfield{author}{\bibinfo{person}{Hu et al.}}
  \bibinfo{year}{2024}\natexlab{s}.
\newblock \showarticletitle{No Free Lunch: Retrieval-Augmented Generation
  Undermines Fairness in LLMs, Even for Vigilant Users}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2410.07589}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Huang et al.}}
  \bibinfo{year}{2024}\natexlab{t}.
\newblock \showarticletitle{TrustLLM: Trustworthiness in Large Language
  Models}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2401.05561}

\bibitem[et~al.(2025i)]%
        {hall2025guiding}
\bibfield{author}{\bibinfo{person}{Hall et al.}}
  \bibinfo{year}{2025}\natexlab{i}.
\newblock \showarticletitle{Guiding LLM Decision-Making with Fairness Reward
  Models}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2507.11344}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Hu et al.}}
  \bibinfo{year}{2025}\natexlab{j}.
\newblock \showarticletitle{LLMs on Trial: Evaluating Judicial Fairness for
  Large Language Models}.
\newblock \bibinfo{journal}{\emph{arXiv}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2507.10852}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Hong et al.}}
  \bibinfo{year}{2025}\natexlab{k}.
\newblock \showarticletitle{Measuring Sycophancy of Language Models in
  Multi-turn Dialogues}.
\newblock  (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2505.23840}
\urldef\tempurl%
...
\bibitem[et~al.(2024u)]%
        {jimenez2024swe}
\bibfield{author}{\bibinfo{person}{Jimenez et al.}}
  \bibinfo{year}{2024}\natexlab{u}.
\newblock \showarticletitle{SWE-bench: Can Language Models Resolve Real-World
  GitHub Issues?}
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2310.06770}

\bibitem[Karimi et~al\mbox{.}(2022)]%
        {karimi2022recourse}
\bibfield{author}{\bibinfo{person}{Amir-Hossein Karimi}, \bibinfo{person}{Gilles
  Barthe}, \bibinfo{person}{Bernhard Sch\"olkopf}, {and} \bibinfo{person}{Isabel
  Valera}.} \bibinfo{year}{2022}\natexlab{}.
\newblock \showarticletitle{A Survey of Algorithmic Recourse: Definitions,
  Formulations, Solutions, and Prospects}.
\newblock \bibinfo{journal}{\emph{ACM Computing Surveys}}
  (\bibinfo{year}{2022}).
\newblock
\showeprint[arxiv]{2010.04050}

\bibitem[et~al.(2017b)]%
        {kusner2017counterfactual}
\bibfield{author}{\bibinfo{person}{Kusner et al.}}
  \bibinfo{year}{2017}\natexlab{b}.
\newblock \showarticletitle{Counterfactual Fairness}.
\newblock  (\bibinfo{year}{2017}).
\newblock
\showeprint[arxiv]{1703.06856}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Kleinberg et al.}}
  \bibinfo{year}{2017}\natexlab{c}.
\newblock \showarticletitle{Inherent Trade-Offs in the Fair Determination of
  Risk Scores}.
\newblock  (\bibinfo{year}{2017}).
\newblock
\showeprint[arxiv]{1609.05807}

\bibitem[et~al.(2024v)]%
        {kantharuban2024stereotype}
\bibfield{author}{\bibinfo{person}{Kantharuban et al.}}
  \bibinfo{year}{2024}\natexlab{v}.
\newblock \showarticletitle{Stereotype or Personalization? User Identity Biases
  Chatbot Recommendations}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2410.05613}
\urldef\tempurl%
...
\bibitem[et~al.(2025l)]%
        {kim2025mitigating}
\bibfield{author}{\bibinfo{person}{Kim et al.}}
  \bibinfo{year}{2025}\natexlab{l}.
\newblock \showarticletitle{Mitigating Bias in RAG: Controlling the Embedder}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2502.17390}
\urldef\tempurl%
...
\bibitem[et~al.(2025m)]%
        {ki2025multiple}
\bibfield{author}{\bibinfo{person}{Ki et al.}}
  \bibinfo{year}{2025}\natexlab{m}.
\newblock \showarticletitle{Multiple LLM Agents Debate for Equitable Cultural
  Alignment}.
\newblock  (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2505.24671}
\urldef\tempurl%
...
\bibitem[et~al.(2025n)]%
        {kabra2025reasoning}
\bibfield{author}{\bibinfo{person}{Kabra et al.}}
  \bibinfo{year}{2025}\natexlab{n}.
\newblock \showarticletitle{Reasoning Towards Fairness: Mitigating Bias in
  Language Models through Reasoning-Guided Fine-Tuning}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2504.05632}
\urldef\tempurl%
...
\bibitem[et~al.(2020c)]%
        {lewis2020retrieval}
\bibfield{author}{\bibinfo{person}{Lewis et al.}}
  \bibinfo{year}{2020}\natexlab{c}.
\newblock \showarticletitle{Retrieval-Augmented Generation for
  Knowledge-Intensive NLP Tasks}.
\newblock  (\bibinfo{year}{2020}).
\newblock
\showeprint[arxiv]{2005.11401}
\urldef\tempurl%
...
\bibitem[et~al.(2023f)]%
        {liang2023holistic}
\bibfield{author}{\bibinfo{person}{Liang et al.}}
  \bibinfo{year}{2023}\natexlab{f}.
\newblock \showarticletitle{Holistic Evaluation of Language Models}.
\newblock  (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2211.09110}

\bibitem[et~al.(2023g)]%
        {liu2023lost}
\bibfield{author}{\bibinfo{person}{Liu et al.}}
  \bibinfo{year}{2023}\natexlab{g}.
\newblock \showarticletitle{Lost in the Middle: How Language Models Use Long
  Contexts}.
\newblock  (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2307.03172}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Liu et al.}}
  \bibinfo{year}{2024}\natexlab{w}.
\newblock \showarticletitle{AgentBench: Evaluating LLMs as Agents}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2308.03688}

\bibitem[et~al.(2024x)]%
        {lin2024assessing}
\bibfield{author}{\bibinfo{person}{Lin et al.}}
  \bibinfo{year}{2024}\natexlab{x}.
\newblock \showarticletitle{Assessing Dialect Fairness and Robustness of Large
  Language Models in Reasoning Tasks}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2410.11005}
\urldef\tempurl%
...
\bibitem[et~al.(2024y)]%
        {li2024prompting}
\bibfield{author}{\bibinfo{person}{Li et al.}}
  \bibinfo{year}{2024}\natexlab{y}.
\newblock \showarticletitle{Prompting Fairness: Integrating Causality to Debias
  Large Language Models}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2403.08743}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Li et al.}}
  \bibinfo{year}{2025}\natexlab{o}.
\newblock \showarticletitle{Actions Speak Louder than Words: Agent Decisions
  Reveal Implicit Biases in Language Models}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2501.17420}
\href{https://doi.org/10.48550/arXiv.2501.17420}{doi:\nolinkurl{10.48550/arXiv.2501.17420}}

\bibitem[et~al.(2025p)]%
        {li2025fairsteer}
\bibfield{author}{\bibinfo{person}{Li et al.}}
  \bibinfo{year}{2025}\natexlab{p}.
\newblock \showarticletitle{FairSteer: Inference Time Debiasing for LLMs with
  Dynamic Activation Steering}.
\newblock  (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2504.14492}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Li et al.}}
  \bibinfo{year}{2025}\natexlab{q}.
\newblock \showarticletitle{From Single to Societal: Analyzing Persona-Induced
  Bias in Multi-Agent Interactions}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2511.11789}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Liu et al.}}
  \bibinfo{year}{2025}\natexlab{r}.
\newblock \showarticletitle{Truth Decay: Quantifying Multi-Turn Sycophancy in
  Language Models}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2503.11656}
\urldef\tempurl%
...
\bibitem[et~al.(2009)]%
        {moher2009preferred}
\bibfield{author}{\bibinfo{person}{Moher et al.}}
  \bibinfo{year}{2009}\natexlab{}.
\newblock \showarticletitle{Preferred Reporting Items for Systematic Reviews
  and Meta-Analyses: The PRISMA Statement}.
\newblock  (\bibinfo{year}{2009}).
\newblock
\href{https://doi.org/10.1371/journal.pmed.1000097}{doi:\nolinkurl{10.1371/journal.pmed.1000097}}

\bibitem[et~al.(2020d)]%
        {mansoury2020feedback}
\bibfield{author}{\bibinfo{person}{Mansoury et al.}}
  \bibinfo{year}{2020}\natexlab{d}.
\newblock \showarticletitle{Feedback Loop and Bias Amplification in Recommender
  Systems}.
\newblock  (\bibinfo{year}{2020}).
\newblock
\showeprint[arxiv]{2007.13019}
\urldef\tempurl%
...
\bibitem[et~al.(2021c)]%
        {makhortykh2021detecting}
\bibfield{author}{\bibinfo{person}{Makhortykh et al.}}
  \bibinfo{year}{2021}\natexlab{c}.
\newblock \showarticletitle{Detecting race and gender bias in visual
  representation of AI on web search engines}.
\newblock  (\bibinfo{year}{2021}).
\newblock
\showeprint[arxiv]{2106.14072}
\href{https://doi.org/10.1007/978-3-030-78818-6_5}{doi:\nolinkurl{10.1007/978-3-030-78818-6_5}}

\bibitem[et~al.(2021d)]%
        {mehrabi2021survey}
\bibfield{author}{\bibinfo{person}{Mehrabi et al.}}
  \bibinfo{year}{2021}\natexlab{d}.
\newblock \showarticletitle{A Survey on Bias and Fairness in Machine Learning}.
\newblock  (\bibinfo{year}{2021}).
\newblock
\showeprint[arxiv]{1908.09635}
\href{https://doi.org/10.1145/3457607}{doi:\nolinkurl{10.1145/3457607}}

\bibitem[et~al.(2025s)]%
        {madigan2025emergent}
\bibfield{author}{\bibinfo{person}{Madigan et al.}}
  \bibinfo{year}{2025}\natexlab{s}.
\newblock \showarticletitle{Emergent Bias and Fairness in Multi-Agent Decision
  Systems}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2512.16433}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Mohammadi et al.}}
  \bibinfo{year}{2025}\natexlab{t}.
\newblock \showarticletitle{Evaluation and Benchmarking of LLM Agents: A
  Survey}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2507.21504}
\urldef\tempurl%
...
\bibitem[et~al.(2025u)]%
        {mirza2025malibu}
\bibfield{author}{\bibinfo{person}{Mirza et al.}}
  \bibinfo{year}{2025}\natexlab{u}.
\newblock \showarticletitle{MALIBU Benchmark: Multi-Agent LLM Implicit Bias
  Uncovered}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2507.01019}
\urldef\tempurl%
...
\bibitem[et~al.(2025v)]%
        {mire2025rejected}
\bibfield{author}{\bibinfo{person}{Mire et al.}}
  \bibinfo{year}{2025}\natexlab{v}.
\newblock \showarticletitle{Rejected Dialects: Biases Against African American
  Language in Reward Models}.
\newblock  (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2502.12858}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Mayilvaghanan et al.}}
  \bibinfo{year}{2026}\natexlab{d}.
\newblock \showarticletitle{Counterfactual Fairness Evaluation of LLM-Based
  Contact Center Agent Quality Assurance System}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2026}).
\newblock
\showeprint[arxiv]{2602.14970}
\urldef\tempurl%
...
\bibitem[et~al.(2026e)]%
        {ma2026implicit}
\bibfield{author}{\bibinfo{person}{Ma et al.}}
  \bibinfo{year}{2026}\natexlab{e}.
\newblock \showarticletitle{How Implicit Bias Accumulates and Propagates in LLM
  Long-term Memory}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2026}).
\newblock
\showeprint[arxiv]{2602.01558}
\urldef\tempurl%
...
\bibitem[et~al.(2020e)]%
        {nangia2020crows}
\bibfield{author}{\bibinfo{person}{Nangia et al.}}
  \bibinfo{year}{2020}\natexlab{e}.
\newblock \showarticletitle{CrowS-Pairs: A Challenge Dataset for Measuring
  Social Biases in Masked Language Models}.
\newblock  (\bibinfo{year}{2020}).
\newblock
\showeprint[arxiv]{2010.00133}
\urldef\tempurl%
...
\bibitem[et~al.(2021e)]%
        {nadeem2021stereoset}
\bibfield{author}{\bibinfo{person}{Nadeem et al.}}
  \bibinfo{year}{2021}\natexlab{e}.
\newblock \showarticletitle{StereoSet: Measuring Stereotypical Bias in
  Pretrained Language Models}.
\newblock  (\bibinfo{year}{2021}).
\newblock
\showeprint[arxiv]{2004.09456}
\urldef\tempurl%
...
\bibitem[et~al.(2023h)]%
        {navigli2023biases}
\bibfield{author}{\bibinfo{person}{Navigli et al.}}
  \bibinfo{year}{2023}\natexlab{h}.
\newblock \showarticletitle{Biases in Large Language Models: Origins,
  Inventory, and Discussion}.
\newblock  (\bibinfo{year}{2023}).
\newblock
\href{https://doi.org/10.1145/3597307}{doi:\nolinkurl{10.1145/3597307}}

\bibitem[et~al.(2024z)]%
        {nghiem2024you}
\bibfield{author}{\bibinfo{person}{Nghiem et al.}}
  \bibinfo{year}{2024}\natexlab{z}.
\newblock \showarticletitle{"You Gotta be a Doctor, Lin": An Investigation of
  Name-Based Bias of Large Language Models in Employment Recommendations}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2406.12232}
\urldef\tempurl%
...
\bibitem[et~al.(2025w)]%
        {nguyen2025social}
\bibfield{author}{\bibinfo{person}{Nguyen et al.}}
  \bibinfo{year}{2025}\natexlab{w}.
\newblock \showarticletitle{The Social Cost of Intelligence: Emergence,
  Propagation, and Amplification of Stereotypical Bias in Multi-Agent Systems}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2510.10943}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Obermeyer et al.}}
  \bibinfo{year}{2019}\natexlab{a}.
\newblock \showarticletitle{Dissecting Racial Bias in an Algorithm Used to
  Manage the Health of Populations}.
\newblock  (\bibinfo{year}{2019}).
\newblock
\href{https://doi.org/10.1126/science.aax2342}{doi:\nolinkurl{10.1126/science.aax2342}}

\bibitem[et~al.(2022c)]%
        {ouyang2022training}
\bibfield{author}{\bibinfo{person}{Ouyang et al.}}
  \bibinfo{year}{2022}\natexlab{c}.
\newblock \showarticletitle{Training language models to follow instructions
  with human feedback}.
\newblock  (\bibinfo{year}{2022}).
\newblock
\showeprint[arxiv]{2203.02155}
\urldef\tempurl%
...
\bibitem[et~al.(2024)]%
        {owens2024multi}
\bibfield{author}{\bibinfo{person}{Owens et al.}}
  \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{A Multi-LLM Debiasing Framework}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2409.13884}
\urldef\tempurl%
...
\bibitem[et~al.(2025x)]%
        {omar2025sociodemographic}
\bibfield{author}{\bibinfo{person}{Omar et al.}}
  \bibinfo{year}{2025}\natexlab{x}.
\newblock \showarticletitle{Sociodemographic biases in medical decision making
  by large language models}.
\newblock  (\bibinfo{year}{2025}).
\newblock
\href{https://doi.org/10.1038/s41591-025-03626-6}{doi:\nolinkurl{10.1038/s41591-025-03626-6}}

\bibitem[Packer et~al\mbox{.}(2023)]%
        {packer2023memgpt}
\bibfield{author}{\bibinfo{person}{Charles Packer}, \bibinfo{person}{Sarah
  Wooders}, \bibinfo{person}{Kevin Lin}, {et~al\mbox{.}}}
  \bibinfo{year}{2023}\natexlab{}.
\newblock \showarticletitle{MemGPT: Towards LLMs as Operating Systems}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2310.08560}

\bibitem[et~al.(2022d)]%
        {parrish2022bbq}
\bibfield{author}{\bibinfo{person}{Parrish et al.}}
  \bibinfo{year}{2022}\natexlab{d}.
\newblock \showarticletitle{BBQ: A Hand-Built Bias Benchmark for Question
  Answering}.
\newblock  (\bibinfo{year}{2022}).
\newblock
\showeprint[arxiv]{2110.08193}
\urldef\tempurl%
...
\bibitem[et~al.(2023i)]%
        {park2023generative}
\bibfield{author}{\bibinfo{person}{Park et al.}}
  \bibinfo{year}{2023}\natexlab{i}.
\newblock \showarticletitle{Generative Agents: Interactive Simulacra of Human
  Behavior}.
\newblock  (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2304.03442}
\href{https://doi.org/10.1145/3586183.3606763}{doi:\nolinkurl{10.1145/3586183.3606763}}

\bibitem[et~al.(2023j)]%
        {pezeshkpour2023large}
\bibfield{author}{\bibinfo{person}{Pezeshkpour et al.}}
  \bibinfo{year}{2023}\natexlab{j}.
\newblock \showarticletitle{Large Language Models Sensitivity to The Order of
  Options in Multiple-Choice Questions}.
\newblock \bibinfo{journal}{\emph{ArXiv preprint}} (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2308.11483}
\urldef\tempurl%
...
\bibitem[et~al.(2024)]%
        {poulain2024bias}
\bibfield{author}{\bibinfo{person}{Poulain et al.}}
  \bibinfo{year}{2024}\natexlab{|}.
\newblock \showarticletitle{Bias patterns in the application of LLMs for
  clinical decision support: A comprehensive study}.
\newblock \bibinfo{journal}{\emph{arXiv}} (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2404.15149}
\urldef\tempurl%
...
\bibitem[et~al.(2024)]%
        {pfohl2024toolbox}
\bibfield{author}{\bibinfo{person}{Pfohl et al.}}
  \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{A Toolbox for Surfacing Health Equity Harms and
  Biases in Large Language Models}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2403.12025}
\href{https://doi.org/10.1038/s41591-024-03258-2}{doi:\nolinkurl{10.1038/s41591-024-03258-2}}

\bibitem[et~al.(2025y)]%
        {pawar2025presumed}
\bibfield{author}{\bibinfo{person}{Pawar et al.}}
  \bibinfo{year}{2025}\natexlab{y}.
\newblock \showarticletitle{Presumed Cultural Identity: How Names Shape LLM
  Responses}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2502.11995}
\urldef\tempurl%
...
\bibitem[et~al.(2025z)]%
        {parziale2025systematic}
\bibfield{author}{\bibinfo{person}{Parziale et al.}}
  \bibinfo{year}{2025}\natexlab{z}.
\newblock \showarticletitle{Toward Systematic Counterfactual Fairness
  Evaluation of Large Language Models: The CAFFE Framework}.
\newblock  (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2512.16816}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Parziale et al.}}
  \bibinfo{year}{2026}\natexlab{f}.
\newblock \showarticletitle{Once Upon a Team: Investigating Bias in LLM-Driven
  Software Team Composition and Task Allocation}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2026}).
\newblock
\showeprint[arxiv]{2601.03857}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Parziale et al.}}
  \bibinfo{year}{2026}\natexlab{g}.
\newblock \showarticletitle{SCOPE: A Dataset of Stereotyped Prompts for
  Counterfactual Fairness Assessment of LLMs}.
\newblock  (\bibinfo{year}{2026}).
\newblock
\showeprint[arxiv]{2604.05555}
\urldef\tempurl%
...
\bibitem[et~al.(2024)]%
        {qian2024chatdev}
\bibfield{author}{\bibinfo{person}{Qian et al.}}
  \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{ChatDev: Communicative Agents for Software
  Development}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2307.07924}

\bibitem[et~al.(2024)]%
        {qu2024tool}
\bibfield{author}{\bibinfo{person}{Qu et al.}} \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{Tool Learning with Large Language Models: A
  Survey}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2405.17935}

\bibitem[et~al.(2018a)]%
        {rudinger2018gender}
\bibfield{author}{\bibinfo{person}{Rudinger et al.}}
  \bibinfo{year}{2018}\natexlab{a}.
\newblock \showarticletitle{Gender Bias in Coreference Resolution
  (WinoGender)}.
\newblock  (\bibinfo{year}{2018}).
\newblock
\showeprint[arxiv]{1804.09301}

\bibitem[et~al.(2020f)]%
        {raghavan2020mitigating}
\bibfield{author}{\bibinfo{person}{Raghavan et al.}}
  \bibinfo{year}{2020}\natexlab{f}.
\newblock \showarticletitle{Mitigating Bias in Algorithmic Hiring: Evaluating
  Claims and Practices}.
\newblock  (\bibinfo{year}{2020}).
\newblock
\href{https://doi.org/10.1145/3351095.3372828}{doi:\nolinkurl{10.1145/3351095.3372828}}

\bibitem[et~al.(2019b)]%
        {selbst2019fairness}
\bibfield{author}{\bibinfo{person}{Selbst et al.}}
  \bibinfo{year}{2019}\natexlab{b}.
\newblock \showarticletitle{Fairness and Abstraction in Sociotechnical
  Systems}.
\newblock  (\bibinfo{year}{2019}).
\newblock
\href{https://doi.org/10.1145/3287560.3287598}{doi:\nolinkurl{10.1145/3287560.3287598}}

\bibitem[et~al.(2022e)]%
        {smith2022sorry}
\bibfield{author}{\bibinfo{person}{Smith et al.}}
  \bibinfo{year}{2022}\natexlab{e}.
\newblock \showarticletitle{"I'm sorry to hear that": Finding New Biases in
  Language Models with a Holistic Descriptor Dataset}.
\newblock  (\bibinfo{year}{2022}).
\newblock
\showeprint[arxiv]{2205.09209}
\urldef\tempurl%
...
\bibitem[et~al.(2023k)]%
        {staab2023memorization}
\bibfield{author}{\bibinfo{person}{Staab et al.}}
  \bibinfo{year}{2023}\natexlab{k}.
\newblock \showarticletitle{Beyond Memorization: Violating Privacy Via
  Inference with Large Language Models}.
\newblock \bibinfo{journal}{\emph{arXiv preprint (ICLR 2024 submission)}}
  (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2310.07298}
\urldef\tempurl%
...
\bibitem[et~al.(2023l)]%
        {shinn2023reflexion}
\bibfield{author}{\bibinfo{person}{Shinn et al.}}
  \bibinfo{year}{2023}\natexlab{l}.
\newblock \showarticletitle{Reflexion: Language Agents with Verbal
  Reinforcement Learning}.
\newblock  (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2303.11366}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Schick et al.}}
  \bibinfo{year}{2023}\natexlab{m}.
\newblock \showarticletitle{Toolformer: Language Models Can Teach Themselves to
  Use Tools}.
\newblock  (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2302.04761}
\urldef\tempurl%
...
\bibitem[et~al.(2023n)]%
        {santurkar2023whose}
\bibfield{author}{\bibinfo{person}{Santurkar et al.}}
  \bibinfo{year}{2023}\natexlab{n}.
\newblock \showarticletitle{Whose Opinions Do Language Models Reflect?}
\newblock  (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2303.17548}

\bibitem[et~al.(2024)]%
        {sumers2024cognitive}
\bibfield{author}{\bibinfo{person}{Sumers et al.}}
  \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{Cognitive Architectures for Language Agents}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2309.02427}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Sharma et al.}}
  \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{Towards Understanding Sycophancy in Language
  Models}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2310.13548}

\bibitem[et~al.(2025)]%
        {singh2025bias}
\bibfield{author}{\bibinfo{person}{Singh et al.}}
  \bibinfo{year}{2025}\natexlab{}.
\newblock \showarticletitle{Bias-Aware Agent: Enhancing Fairness in AI-Driven
  Knowledge Retrieval}.
\newblock  (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2503.21237}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Singh et al.}}
  \bibinfo{year}{2025}\natexlab{|}.
\newblock \showarticletitle{Bias Mitigation Agent: Optimizing Source Selection
  for Fair and Balanced Knowledge Retrieval}.
\newblock  (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2508.18724}
\urldef\tempurl%
...
\bibitem[et~al.(2025)]%
        {she2025fairsense}
\bibfield{author}{\bibinfo{person}{She et al.}}
  \bibinfo{year}{2025}\natexlab{}.
\newblock \showarticletitle{FairSense: Long-Term Fairness Analysis of
  ML-Enabled Systems}.
\newblock  (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2501.01665}
\href{https://doi.org/10.1109/ICSE55347.2025.00159}{doi:\nolinkurl{10.1109/ICSE55347.2025.00159}}

\bibitem[et~al.(2025)]%
        {sneh2025tooltweak}
\bibfield{author}{\bibinfo{person}{Sneh et al.}}
  \bibinfo{year}{2025}\natexlab{}.
\newblock \showarticletitle{ToolTweak: An Attack on Tool Selection in LLM-based
  Agents}.
\newblock \bibinfo{journal}{\emph{ArXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2510.02554}
\urldef\tempurl%
...
\bibitem[et~al.(2025)]%
        {song2025large}
\bibfield{author}{\bibinfo{person}{Song et al.}}
  \bibinfo{year}{2025}\natexlab{}.
\newblock \showarticletitle{Towards Large Language Models that Benefit for All:
  Benchmarking Group Fairness in Reward Models}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2503.07806}
\urldef\tempurl%
...
\bibitem[et~al.(2026h)]%
        {simhi2026old}
\bibfield{author}{\bibinfo{person}{Simhi et al.}}
  \bibinfo{year}{2026}\natexlab{h}.
\newblock \showarticletitle{Old Habits Die Hard: How Conversational History
  Geometrically Traps LLMs}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2026}).
\newblock
\showeprint[arxiv]{2603.03308}
\urldef\tempurl%
...
\bibitem[et~al.(2023o)]%
        {turpin2023language}
\bibfield{author}{\bibinfo{person}{Turpin et al.}}
  \bibinfo{year}{2023}\natexlab{o}.
\newblock \showarticletitle{Language Models Don't Always Say What They Think:
  Unfaithful Explanations in Chain-of-Thought Prompting}.
\newblock  (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2305.04388}

\bibitem[et~al.(2026i)]%
        {tonneau2026different}
\bibfield{author}{\bibinfo{person}{Tonneau et al.}}
  \bibinfo{year}{2026}\natexlab{i}.
\newblock \showarticletitle{Different Demographic Cues Yield Inconsistent
  Conclusions About LLM Personalization and Bias}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2026}).
\newblock
\showeprint[arxiv]{2601.18486}
\urldef\tempurl%
...
\bibitem[et~al.(2026j)]%
        {tang2026your}
\bibfield{author}{\bibinfo{person}{Tang et al.}}
  \bibinfo{year}{2026}\natexlab{j}.
\newblock \showarticletitle{Is Your LLM-as-a-Recommender Agent Trustable? LLMs'
  Recommendation is Easily Hacked by Biases (Preferences)}.
\newblock \bibinfo{journal}{\emph{ArXiv preprint}} (\bibinfo{year}{2026}).
\newblock
\showeprint[arxiv]{2603.17417}
\urldef\tempurl%
...
\bibitem[et~al.(2023p)]%
        {veldanda2023emily}
\bibfield{author}{\bibinfo{person}{Veldanda et al.}}
  \bibinfo{year}{2023}\natexlab{p}.
\newblock \showarticletitle{Are Emily and Greg Still More Employable than
  Lakisha and Jamal? Investigating Algorithmic Hiring Bias in the Era of
  ChatGPT}.
\newblock \bibinfo{journal}{\emph{EMNLP 2024 (arXiv preprint 2023)}}
  (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2310.05135}
\urldef\tempurl%
...
\bibitem[et~al.(2024)]%
        {vijjini2024exploring}
\bibfield{author}{\bibinfo{person}{Vijjini et al.}}
  \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{Exploring Safety-Utility Trade-Offs in Personalized
  Language Models}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2406.11107}
\urldef\tempurl%
...
\bibitem[et~al.(2026k)]%
        {vatsal2026agentic}
\bibfield{author}{\bibinfo{person}{Vatsal et al.}}
  \bibinfo{year}{2026}\natexlab{k}.
\newblock \showarticletitle{Agentic AI in Healthcare \& Medicine: A
  Seven-Dimensional Taxonomy for Empirical Evaluation of LLM-based Agents}.
\newblock  (\bibinfo{year}{2026}).
\newblock
\showeprint[arxiv]{2602.04813}
\urldef\tempurl%
...
\bibitem[et~al.(2023q)]%
        {wu2023autogen}
\bibfield{author}{\bibinfo{person}{Wu et al.}}
  \bibinfo{year}{2023}\natexlab{q}.
\newblock \showarticletitle{AutoGen: Enabling Next-Gen LLM Applications via
  Multi-Agent Conversation}.
\newblock \bibinfo{journal}{\emph{arXiv preprint (ICLR 2024 workshop)}}
  (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2308.08155}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Wang et al.}}
  \bibinfo{year}{2023}\natexlab{r}.
\newblock \showarticletitle{DecodingTrust: A Comprehensive Assessment of
  Trustworthiness in GPT Models}.
\newblock  (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2306.11698}

\bibitem[et~al.(2024)]%
        {wang2024ceb}
\bibfield{author}{\bibinfo{person}{Wang et al.}}
  \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{CEB: Compositional Evaluation Benchmark for
  Fairness in Large Language Models}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2407.02408}
\urldef\tempurl%
...
\bibitem[et~al.(2024)]%
        {wu2024rag}
\bibfield{author}{\bibinfo{person}{Wu et al.}} \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{Does RAG Introduce Unfairness in LLMs? Evaluating
  Fairness in Retrieval-Augmented Generation Systems}.
\newblock \bibinfo{journal}{\emph{ArXiv preprint}} (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2409.19804}
\urldef\tempurl%
...
\bibitem[et~al.(2024)]%
        {wyllie2024fairness}
\bibfield{author}{\bibinfo{person}{Wyllie et al.}}
  \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{Fairness Feedback Loops: Training on Synthetic Data
  Amplifies Bias}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2403.07857}
\href{https://doi.org/10.1145/3630106.3659029}{doi:\nolinkurl{10.1145/3630106.3659029}}

\bibitem[et~al.(2024)]%
        {wu2024fmbench}
\bibfield{author}{\bibinfo{person}{Wu et al.}} \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{FMBench: Benchmarking Fairness in Multimodal Large
  Language Models on Medical Tasks}.
\newblock \bibinfo{journal}{\emph{arXiv preprint (ICLR 2025 workshop)}}
  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2410.01089}
\urldef\tempurl%
...
\bibitem[et~al.(2024)]%
        {wilson2024gender}
\bibfield{author}{\bibinfo{person}{Wilson et al.}}
  \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{Gender, Race, and Intersectional Bias in Resume
  Screening via Language Model Retrieval}.
\newblock \bibinfo{journal}{\emph{arXiv}} (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2407.20371}
\urldef\tempurl%
...
\bibitem[et~al.(2024)]%
        {weissburg2024llms}
\bibfield{author}{\bibinfo{person}{Weissburg et al.}}
  \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{LLMs are Biased Teachers: Evaluating LLM Bias in
  Personalized Education}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2410.14012}
\urldef\tempurl%
...
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2308.11432}
\href{https://doi.org/10.1007/s11704-024-40231-1}{doi:\nolinkurl{10.1007/s11704-024-40231-1}}

\bibitem[et~al.(2025)]%
        {wang2025bias}
\bibfield{author}{\bibinfo{person}{Wang et al.}}
  \bibinfo{year}{2025}\natexlab{}.
\newblock \showarticletitle{Bias Amplification in RAG: Poisoning Knowledge
  Retrieval to Steer LLMs}.
\newblock \bibinfo{journal}{\emph{ArXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2506.11415}
\urldef\tempurl%
...
\bibitem[et~al.(2025)]%
        {wu2025reasoning}
\bibfield{author}{\bibinfo{person}{Wu et al.}} \bibinfo{year}{2025}\natexlab{}.
\newblock \showarticletitle{Does Reasoning Introduce Bias? A Study of Social
  Bias Evaluation and Mitigation in LLM Reasoning}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2502.15361}
\urldef\tempurl%
...
\bibitem[et~al.(2026l)]%
        {wang2026observations}
\bibfield{author}{\bibinfo{person}{Wang et al.}}
  \bibinfo{year}{2026}\natexlab{l}.
\newblock \showarticletitle{Observations and Remedies for Large Language Model
  Bias in Self-Consuming Performative Loop}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2026}).
\newblock
\showeprint[arxiv]{2601.05184}
\urldef\tempurl%
...
\bibitem[et~al.(2023s)]%
        {xi2023rise}
\bibfield{author}{\bibinfo{person}{Xi et al.}}
  \bibinfo{year}{2023}\natexlab{s}.
\newblock \showarticletitle{The Rise and Potential of Large Language Model
  Based Agents: A Survey}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2309.07864}
\urldef\tempurl%
...
\bibitem[et~al.(2024)]%
        {xu2024adapting}
\bibfield{author}{\bibinfo{person}{Xu et al.}} \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{Adapting Static Fairness to Sequential
  Decision-Making: Bias Mitigation Strategies towards Equal Long-term Benefit
  Rate}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2309.03426}
\urldef\tempurl%
...
\bibitem[et~al.(2024)]%
        {xu2024mitigating}
\bibfield{author}{\bibinfo{person}{Xu et al.}} \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{Mitigating Social Bias in Large Language Models: A
  Multi-Objective Approach within a Multi-Agent Framework}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2412.15504}
\urldef\tempurl%
...
\bibitem[et~al.(2024)]%
        {xiao2024algorithmic}
\bibfield{author}{\bibinfo{person}{Xiao et al.}}
  \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{On the Algorithmic Bias of Aligning Large Language
  Models with RLHF: Preference Collapse and Matching Regularization}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2405.16455}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Xiao et al.}}
  \bibinfo{year}{2025}\natexlab{}.
\newblock \showarticletitle{FairMedQA: Benchmarking Bias in Large Language
  Models for Medical Question Answering}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2505.19562}
\urldef\tempurl%
...
\bibitem[et~al.(2025)]%
        {xu2025quantifying}
\bibfield{author}{\bibinfo{person}{Xu et al.}} \bibinfo{year}{2025}\natexlab{}.
\newblock \showarticletitle{Quantifying Fairness in LLMs Beyond Tokens: A
  Semantic and Statistical Perspective}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2506.19028}
\urldef\tempurl%
...
\bibitem[et~al.(2026m)]%
        {xu2026ducx}
\bibfield{author}{\bibinfo{person}{Xu et al.}}
  \bibinfo{year}{2026}\natexlab{m}.
\newblock \showarticletitle{DUCX: Decomposing Unfairness in Tool-Using Chest
  X-ray Agents}.
\newblock \bibinfo{journal}{\emph{ArXiv preprint}} (\bibinfo{year}{2026}).
\newblock
\showeprint[arxiv]{2603.00777}
\urldef\tempurl%
...
\bibitem[et~al.(2023t)]%
        {yao2023react}
\bibfield{author}{\bibinfo{person}{Yao et al.}}
  \bibinfo{year}{2023}\natexlab{t}.
\newblock \showarticletitle{ReAct: Synergizing Reasoning and Acting in Language
  Models}.
\newblock  (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2210.03629}
\urldef\tempurl%
...
\bibitem[et~al.(2025)]%
        {yu2025survey}
\bibfield{author}{\bibinfo{person}{Yu et al.}} \bibinfo{year}{2025}\natexlab{}.
\newblock \showarticletitle{A Survey on Trustworthy LLM Agents: Threats and
  Countermeasures}.
\newblock  (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2503.09648}
\urldef\tempurl%
...
\bibitem[et~al.(2018b)]%
        {zhao2018gender}
\bibfield{author}{\bibinfo{person}{Zhao et al.}}
  \bibinfo{year}{2018}\natexlab{b}.
\newblock \showarticletitle{Gender Bias in Coreference Resolution: Evaluation
  and Debiasing Methods (WinoBias)}.
\newblock  (\bibinfo{year}{2018}).
\newblock
\showeprint[arxiv]{1804.06876}

\bibitem[et~al.(2022f)]%
        {zehlike2022fairness}
\bibfield{author}{\bibinfo{person}{Zehlike et al.}}
  \bibinfo{year}{2022}\natexlab{f}.
\newblock \showarticletitle{Fairness in Ranking: A Survey}.
\newblock  (\bibinfo{year}{2022}).
\newblock
\showeprint[arxiv]{2103.14000}
\href{https://doi.org/10.1145/3533379}{doi:\nolinkurl{10.1145/3533379}}

\bibitem[et~al.(2024)]%
        {zhang2024causal}
\bibfield{author}{\bibinfo{person}{Zhang et al.}}
  \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{Causal Prompting: Debiasing Large Language Model
  Prompting based on Front-Door Adjustment}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2403.02738}
\urldef\tempurl%
...
\bibitem[et~al.(2024)]%
        {zack2024coding}
\bibfield{author}{\bibinfo{person}{Zack et al.}}
  \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{Coding Inequity: Assessing GPT-4's Potential for
  Perpetuating Racial and Gender Biases in Healthcare}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\href{https://doi.org/10.1016/S2589-7500(23)00225-X}{doi:\nolinkurl{10.1016/S2589-7500(23)00225-X}}

\bibitem[et~al.(2024)]%
        {zhou2024webarena}
\bibfield{author}{\bibinfo{person}{Zhou et al.}}
  \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{WebArena: A Realistic Web Environment for Building
  Autonomous Agents}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2307.13854}

\bibitem[et~al.(2025)]%
        {zhang2025datasets}
\bibfield{author}{\bibinfo{person}{Zhang et al.}}
  \bibinfo{year}{2025}\natexlab{}.
\newblock \showarticletitle{Datasets for Fairness in Language Models: An
  In-Depth Survey}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2506.23411}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Zhang et al.}}
  \bibinfo{year}{2025}\natexlab{}.
\newblock \showarticletitle{Evaluating the Effect of Retrieval Augmentation on
  Social Biases}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2502.17611}
\urldef\tempurl%
...
\bibfield{author}{\bibinfo{person}{Zhou et al.}}
  \bibinfo{year}{2025}\natexlab{}.
\newblock \showarticletitle{Veracity Bias and Beyond: Uncovering LLMs' Hidden
  Beliefs in Problem-Solving Reasoning}.
\newblock  (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2505.16128}
\urldef\tempurl%
\url{https://arxiv.org/abs/2505.16128}
\showURL{%
\tempurl}

\bibitem[Hu and Zhang(2022)]%
        {hu2022achieving}
\bibfield{author}{\bibinfo{person}{Hu} {and} \bibinfo{person}{Zhang}.}
  \bibinfo{year}{2022}\natexlab{}.
\newblock \showarticletitle{Achieving Long-Term Fairness in Sequential Decision
  Making}.
\newblock  (\bibinfo{year}{2022}).
\newblock
\showeprint[arxiv]{2204.01819}
\urldef\tempurl%
\url{https://arxiv.org/abs/2204.01819}
\showURL{%
\tempurl}

\bibitem[Kambhampati(2024)]%
        {kambhampati2024large}
\bibfield{author}{\bibinfo{person}{Kambhampati}.}
  \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{Can Large Language Models Reason and Plan?}
\newblock  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2403.04121}
\href{https://doi.org/10.1111/nyas.15125}{doi:\nolinkurl{10.1111/nyas.15125}}

\bibitem[Kim and Diaz(2024)]%
        {kim2024fair}
\bibfield{author}{\bibinfo{person}{Kim} {and} \bibinfo{person}{Diaz}.}
  \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{Towards Fair RAG: On the Impact of Fair Ranking in
  Retrieval-Augmented Generation}.
\newblock \bibinfo{journal}{\emph{arXiv preprint (ACM SIGIR ICTIR 2025)}}
  (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2409.11598}
\urldef\tempurl%
...
\bibitem[NIST(2023)]%
        {nist2023artificial}
\bibfield{author}{\bibinfo{person}{NIST}.} \bibinfo{year}{2023}\natexlab{}.
\newblock \showarticletitle{Artificial Intelligence Risk Management Framework
  (AI RMF 1.0)}.
\newblock  (\bibinfo{year}{2023}).
\newblock
\href{https://doi.org/10.6028/NIST.AI.100-1}{doi:\nolinkurl{10.6028/NIST.AI.100-1}}

\bibitem[Parliament and Council(2024)]%
        {parliament2024regulation}
\bibfield{author}{\bibinfo{person}{European Parliament} {and}
  \bibinfo{person}{Council}.} \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{Regulation (EU) 2024/1689 (Artificial Intelligence
  Act)}.
\newblock  (\bibinfo{year}{2024}).
\newblock
\urldef\tempurl%
...
\bibitem[Pessach and Shmueli(2022)]%
        {pessach2022review}
\bibfield{author}{\bibinfo{person}{Dana Pessach} {and} \bibinfo{person}{Eytan
  Shmueli}.} \bibinfo{year}{2022}\natexlab{}.
\newblock \showarticletitle{A Review on Fairness in Machine Learning}.
\newblock  (\bibinfo{year}{2022}).
\newblock
\href{https://doi.org/10.1145/3494672}{doi:\nolinkurl{10.1145/3494672}}

\bibitem[Ranjan et~al\mbox{.}(2025)]%
        {ranjan2025fairness}
\bibfield{author}{\bibinfo{person}{Rajesh Ranjan}, \bibinfo{person}{Shailja
  Gupta}, {and} \bibinfo{person}{Surya~Narayan Singh}.}
  \bibinfo{year}{2025}\natexlab{}.
\newblock \showarticletitle{Fairness in Agentic AI: A Unified Framework for
  Ethical and Equitable Multi-Agent System}.
\newblock \bibinfo{journal}{\emph{arXiv preprint arXiv:2502.07254}}
  (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2502.07254}

\bibitem[Tang et~al\mbox{.}(2023)]%
        {tang2023what}
\bibfield{author}{\bibinfo{person}{Zeyu Tang}, \bibinfo{person}{Jiji Zhang},
  {and} \bibinfo{person}{Kun Zhang}.} \bibinfo{year}{2023}\natexlab{}.
\newblock \showarticletitle{What-is and How-to for Fairness in Machine
  Learning: A Survey, Reflection, and Perspective}.
\newblock  (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2206.04101}
\href{https://doi.org/10.1145/3597199}{doi:\nolinkurl{10.1145/3597199}}

\bibitem[U.(2018)]%
        {u2018algorithms}
\bibfield{author}{\bibinfo{person}{Noble~S. U.}}
  \bibinfo{year}{2018}\natexlab{}.
\newblock \showarticletitle{Algorithms of Oppression: How Search Engines
  Reinforce Racism}.
\newblock  (\bibinfo{year}{2018}).
\newblock
\href{https://doi.org/10.2307/j.ctt1pwt9w5}{doi:\nolinkurl{10.2307/j.ctt1pwt9w5}}

\bibitem[Wang et~al\mbox{.}(2023b)]%
        {wang2023selfconsistency}
\bibfield{author}{\bibinfo{person}{Xuezhi Wang}, \bibinfo{person}{Jason Wei},
  \bibinfo{person}{Dale Schuurmans}, {et~al\mbox{.}}}
  \bibinfo{year}{2023}\natexlab{b}.
\newblock \showarticletitle{Self-Consistency Improves Chain of Thought Reasoning
  in Language Models}.
\newblock \bibinfo{journal}{\emph{International Conference on Learning
  Representations (ICLR)}} (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2203.11171}

\bibitem[Wang et~al\mbox{.}(2023)]%
        {wang2023survey}
\bibfield{author}{\bibinfo{person}{Yifan Wang}, \bibinfo{person}{Weizhi Ma},
  \bibinfo{person}{Min Zhang}, \bibinfo{person}{Yiqun Liu}, {and}
  \bibinfo{person}{Shaoping Ma}.} \bibinfo{year}{2023}\natexlab{}.
\newblock \showarticletitle{A Survey on the Fairness of Recommender Systems}.
\newblock  (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2206.03761}
\href{https://doi.org/10.1145/3547333}{doi:\nolinkurl{10.1145/3547333}}

\bibitem[Xie et~al\mbox{.}(2024)]%
        {xie2024osworld}
\bibfield{author}{\bibinfo{person}{Tianbao Xie}, \bibinfo{person}{Danyang
  Zhang}, \bibinfo{person}{Jixuan Chen}, {et~al\mbox{.}}}
  \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{OSWorld: Benchmarking Multimodal Agents for
  Open-Ended Tasks in Real Computer Environments}.
\newblock \bibinfo{journal}{\emph{Advances in Neural Information Processing
  Systems (NeurIPS)}} (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2404.07972}

\bibitem[Yao et~al\mbox{.}(2023)]%
        {yao2023tot}
\bibfield{author}{\bibinfo{person}{Shunyu Yao}, \bibinfo{person}{Dian Yu},
  \bibinfo{person}{Jeffrey Zhao}, {et~al\mbox{.}}}
  \bibinfo{year}{2023}\natexlab{}.
\newblock \showarticletitle{Tree of Thoughts: Deliberate Problem Solving with
  Large Language Models}.
\newblock \bibinfo{journal}{\emph{Advances in Neural Information Processing
  Systems (NeurIPS)}} (\bibinfo{year}{2023}).
\newblock
\showeprint[arxiv]{2305.10601}

\bibitem[Yao et~al\mbox{.}(2024)]%
        {yao2024taubench}
\bibfield{author}{\bibinfo{person}{Shunyu Yao}, \bibinfo{person}{Noah Shinn},
  \bibinfo{person}{Pedram Razavi}, {and} \bibinfo{person}{Karthik
  Narasimhan}.} \bibinfo{year}{2024}\natexlab{}.
\newblock \showarticletitle{$\tau$-bench: A Benchmark for Tool-Agent-User
  Interaction in Real-World Domains}.
\newblock \bibinfo{journal}{\emph{arXiv preprint}} (\bibinfo{year}{2024}).
\newblock
\showeprint[arxiv]{2406.12045}

\bibitem[Yehudai et~al\mbox{.}(2025)]%
        {yehudai2025survey}
\bibfield{author}{\bibinfo{person}{Asaf Yehudai}, \bibinfo{person}{Lilach
  Eden}, \bibinfo{person}{Alan Li}, \bibinfo{person}{Guy Uziel},
  \bibinfo{person}{Yilun Zhao}, \bibinfo{person}{Roy Bar-Haim},
  \bibinfo{person}{Arman Cohan}, {and} \bibinfo{person}{Michal
  Shmueli-Scheuer}.} \bibinfo{year}{2025}\natexlab{}.
\newblock \showarticletitle{A Survey on Evaluation of LLM-based Agents}.
\newblock \bibinfo{journal}{\emph{arXiv preprint arXiv:2503.16416 (revised
  April 2026)}} (\bibinfo{year}{2025}).
\newblock
\showeprint[arxiv]{2503.16416}
