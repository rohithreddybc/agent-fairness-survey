...
\textbf{Contribution 1: The Bias Conduction Framework (BCF).} The BCF organizes bias in LLM-based agents along two axes: \emph{entry locus} (five architectural components plus a long-horizon temporal axis) and \emph{conduction operator} (ATTENUATE $\varphi < 1$ / PRESERVE $\varphi \approx 1$ / AMPLIFY $\varphi > 1$ / GENERATE $\varphi$ on zero input). The formal core (\S{}3.0) defines: D1 the agent loop as the tuple $\langle$$\pi$,T,M,R$\rangle$ (policy, tool registry, memory module, reward/evaluation signal); D2 component counterfactual disparity $\Delta_c$ measured via trace-replay; D3 the set of conduction operators. The conduction equation $\Delta_\tau$ $\approx$ $\Sigma_c$ $\Delta_c$ $\cdot$ $\Pi$ $\varphi_e$ with feedback recurrence $\Delta^{(t+1)}$ = $\varphi_{\mathrm{fb}}$ $\cdot$ $\Delta^{(t)}$ + $\Delta_{\mathrm{entry}}$ predicts trajectory-level disparity from component measurements. Five propositions follow, four grounded in existing corpus evidence and one (P3's cross-component form) stated as a falsifiable conjecture: P1 Locality (a disparity at component c is attributable to that locus if it vanishes under a component-level counterfactual), P2 Masking (token-level parity can coexist with action-level disparity, the measurement gap stated as a formal consequence of the linearized model), P3 Super-additivity (the agentic multiplier: $\Delta_\tau$ can exceed the naive sum $\Sigma_c$ |$\Delta_c$| when at least one AMPLIFY or GENERATE edge lies on a conduction path), P4 Mitigation matching (an intervention reduces $\Delta_\tau$ only if it targets the locus and operator of the dominant $\Delta_c$), and P5 Measurement adequacy (a fairness metric is adequate for an agent only if it surfaces $\Delta_c$ for every component with non-negligible $\varphi$, with adequacy graded in the tiers of \S{}3.0.5). Section 3.8 provides a worked hiring-agent example tracing bias from C0 (base model) through C3 (delegation) and a four-step auditor recipe (instrument trace-replay, estimate $\varphi_e$ per edge, identify dominant locus, match mitigation to operator).
...
Five architectural components and a long-horizon temporal axis have design choices consequential for fairness:
...
Agent component relevance & Findings bear on at least one of the six loci, or on evaluation/mitigation thereof & Exclusively single-turn static generation with no agentic framing; included only if mechanism transfers to agentic settings (marked \emph{[adjacent]}) \\
...
\end{forest}
\caption{Taxonomy of fairness in LLM-based agents, organized by agent locus
...
Five propositions follow from D1 to D3 and the conduction equation, stated as claims the survey's evidence base supports or motivates, not as theorems about all possible agents. Of the five, P2, P3, and P5 are empirically testable predictions; P1 and P4 are decomposition and attribution principles, asserting where disparity localizes and which interventions can reduce it.

\textbf{P1 (Locality).} \emph{Every measurable trajectory disparity decomposes into component entries plus an interaction residual: $\Delta_\tau$ $\neq$ 0 implies a nonzero $\Delta_c$ at some locus $c$ (counting GENERATE edges as entries), or a nonzero cross-component interaction term that the linearized equation folds into the nearest downstream GENERATE edge.} The residual matters because pure interaction effects, disparity present in no component alone but produced by their composition, are exactly the multi-agent GENERATE phenomenon. P1 is a decomposition and attribution principle rather than an empirical law: the interaction residual books any disparity that no single component explains, so its empirical content is the practical claim, borne out by the cited audits, that component-level counterfactuals localize the disparity that matters. \cite{xu2026ducx} attributes distinct, separable shares of a clinical agent's disparity to the tool-exposure and tool-transition stages versus model reasoning, and the component-specific audits of \S{}\S{}3.1 to 3.5 each isolate a nonzero $\Delta_c$ with other components held fixed.
...
\emph{Takeaway: Five of the six loci have partial or no mitigation coverage; the most consequential operator types (AMPLIFY and GENERATE at agent loci) are the least addressed, and every component's open gap maps directly onto a \S{}8 research agenda item.}
...
This study is deliberately modest in scale and serves three purposes: a \textbf{constructive existence proof} that the P2 Masking gap is empirically measurable and present at this scale, paired with a proxy test of the amplification-ordering prediction associated with P3 (the audit does not estimate the component sum), which is not borne out at this scale; a \textbf{methodological template} (pinned models, synthetic counterfactuals, paired answer/action metrics, BCF-labeled configurations) portable across domains; and a \textbf{design seed} for the acting-agent benchmark called for in \S{}8.1. FairMedAgent, a forthcoming companion benchmark for clinical acting agents evaluated under matched demographic counterfactuals, is one scaled realization of this instrumentation; its development is independent of this survey's claims.
...
\emph{Takeaway: the counterfactual column is near-empty, holding exactly one entry (she2025fairsense) across all six loci, while 52 of the 68 component-branch entries fall in the unspecified column.}
...
P5 Measurement-adequacy requires that a metric detect non-zero $\Delta_c$ at a given component independently of all others. The benchmark inventory in \S{}4.4 shows no existing benchmark satisfies P5 for any of the six agent-pipeline loci at the action level.
...
The audit harness, the counterfactual-swap scripts, the analysis code, and the full set of 144 raw audit decisions reported in \S{}6 are publicly available at \url{https://github.com/rohithreddybc/agent-fairness-survey}. The verified 202-paper corpus and the per-paper BCF coding used to build the coverage map (\S{}7) are included in the same repository.

\section*{Author Contributions}

Author roles follow the CRediT taxonomy. \textbf{Rohith Reddy Bellibaltu}: conceptualization, methodology, formal analysis, software, data curation, investigation, visualization, and writing (original draft, review, and editing). \textbf{Manpreet Singh}: investigation, literature curation, validation, and writing (review and editing). \textbf{Wenbin Zhang}: supervision, methodology, and writing (review and editing). The first author led the framework development, the empirical audit, and manuscript preparation.
...
The authors used generative AI assistants (large language model based writing tools) to support manuscript drafting, copy-editing, LaTeX formatting, and the organization and cross-checking of the surveyed literature. The framework and its formal definitions and propositions, the empirical audit protocol and its analysis, and every citation were conceived, executed, and verified by the authors. All AI-assisted text was reviewed and edited by the authors, who take full responsibility for the content, including its accuracy, originality, and integrity.
